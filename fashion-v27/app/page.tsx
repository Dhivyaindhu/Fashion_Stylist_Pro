...
const PRODUCTS: Record<string,any[]> = {
  Women:[
    ...
    {name:"Anarkali Suit",body:["Apple","Pear","Full Pear","Full Hourglass","Rectangle"],colors:["Deep Burgundy","Cobalt","Jade"],sizes:["S","M","L","XL","XXL","XXXL"],amazon:"https://www.amazon.in/s?k=women+anarkali+suit",flipkart:"https://www.flipkart.com/search?q=women+anarkali"},
    {name:"Printed Saree",body:["Pear","Hourglass","Apple","Rectangle","Full Hourglass","Full Pear"],colors:["Royal Blue","Crimson","Mustard","Teal"],sizes:["Free Size"],amazon:"https://www.amazon.in/s?k=women+printed+saree",flipkart:"https://www.flipkart.com/search?q=women+printed+saree"},
    {name:"Salwar Kameez",body:["Pear","Full Pear","Rectangle","Apple","Hourglass"],colors:["Terracotta","Mustard","Cobalt"],sizes:["XS","S","M","L","XL","XXL","XXXL"],amazon:"https://www.amazon.in/s?k=women+salwar+kameez",flipkart:"https://www.flipkart.com/search?q=women+salwar+kameez"},
    ...
    {name:"Peplum Top",body:["Inverted Triangle","Rectangle"],colors:["Navy","Cobalt","Crimson"],sizes:["XS","S","M","L","XL","XXL"],amazon:"https://www.amazon.in/s?k=women+peplum+top",flipkart:"https://www.flipkart.com/search?q=women+peplum+top"},
  ],
  Men:[
...
// ════════════════════════════════════════════════════════
// AVATAR BUILDER  v28 — all display bugs fixed
//
// FIX 1: INLINE SVG CARTOON FACE — replaces broken DiceBear
//         <image href="dicebear-url"> was loading a LANDSCAPE SVG
//         (the SVG had a sky/grass background built in).
//         Now draws face entirely with inline SVG shapes — zero network calls.
//
// FIX 2: SVG HEIGHT 680→600, y_sh 210→160
//         Old layout had huge blank space at top (head was at y=112,
//         body at y=210, but viewBox was 0-680 → top 30% was empty black).
//         Now body starts at y=160 → fills the frame properly.
//
// FIX 3: iframe sandbox="allow-scripts" — prevents Vercel CSP blocking inline JS
//
// FIX 4: html/body in iframe set to height:100%, overflow:hidden
//         → no scrollbar, no cut-off body
// ════════════════════════════════════════════════════════
function buildAvatar(result: any, dressB64: string|null): string {
  ...
  // ── Canvas dimensions ──────────────────────────────────
  const CX=200, W=400, H=600
  const SC = 4.6  // cm circumference → SVG half-width

  const hw  = (c:number) => Math.max(10, Math.round((c/(2*Math.PI))*SC))
  const sh_w = Math.max(hw((result.shoulder_cm||40)*1.02), 46)
  const bu_w = hw(result.bust_cm||88)
  const wa_w = hw(result.waist_cm||72)
  const hi_w = hw(result.hip_cm||90)
  const th_w = Math.round(hi_w*0.66)
  const ca_w = Math.round(hi_w*0.36)
  const arm_w= Math.max(13, Math.round(sh_w*0.27))
  const nw   = Math.max(11, Math.round(sh_w*0.24))
  const ah   = Math.round(arm_w/2)

  // ── Y positions — body starts at 160 (NOT 210) ─────────
  const y_sh=160, y_bu=y_sh+60, y_wa=y_bu+52, y_hi=y_wa+36
  const y_th=y_hi+70, y_kn=y_th+52, y_ca=y_kn+46, y_ft=y_ca+40
  const y_nek=y_sh-18, y_hcy=y_nek-60

  const skin_sh  = lighten(skin, 0.66)
  const skin_hi  = lighten(skin, 1.34)
  const skin_mid = lighten(skin, 0.82)

  // ── INLINE CARTOON FACE ────────────────────────────────
  const hairC = ({Fair:'#2c1b18',Light:'#3d2314',Medium:'#1a0d00',Tan:'#0d0500',Deep:'#080200'} as any)[skinTone] || '#1a0d00'
  const eyeC  = '#1a0f00'
  const lipC  = lighten(skin, 0.68)
  const R     = 50   // head radius

  // Hair
  const hairFront = isMen
    ? `<path d="M ${CX-R},${y_hcy+10} L ${CX-R-2},${y_hcy-18} Q ${CX-R+4},${y_hcy-R-8} ${CX},${y_hcy-R-12} Q ${CX+R-4},${y_hcy-R-8} ${CX+R+2},${y_hcy-18} L ${CX+R},${y_hcy+10} Z" fill="${hairC}" opacity="0.88"/>`
    : `<path d="M ${CX-R},${y_hcy+10} L ${CX-R-7},${y_hcy+38} Q ${CX-R-5},${y_hcy+56} ${CX-R+3},${y_hcy+62} L ${CX-R+7},${y_hcy+22} Z" fill="${hairC}" opacity="0.84"/>
       <path d="M ${CX+R},${y_hcy+10} L ${CX+R+7},${y_hcy+38} Q ${CX+R+5},${y_hcy+56} ${CX+R-3},${y_hcy+62} L ${CX+R-7},${y_hcy+22} Z" fill="${hairC}" opacity="0.84"/>
       <path d="M ${CX-R},${y_hcy} Q ${CX-R-8},${y_hcy-R-6} ${CX},${y_hcy-R-12} Q ${CX+R+8},${y_hcy-R-6} ${CX+R},${y_hcy} Z" fill="${hairC}" opacity="0.88"/>`

  // Eyelashes (women only)
  const lashes = !isMen ? `
    <line x1="${CX-19}" y1="${y_hcy-12}" x2="${CX-24}" y2="${y_hcy-17}" stroke="${hairC}" stroke-width="1.5" stroke-linecap="round"/>
    <line x1="${CX-12}" y1="${y_hcy-14}" x2="${CX-12}" y2="${y_hcy-20}" stroke="${hairC}" stroke-width="1.5" stroke-linecap="round"/>
    <line x1="${CX+19}" y1="${y_hcy-12}" x2="${CX+24}" y2="${y_hcy-17}" stroke="${hairC}" stroke-width="1.5" stroke-linecap="round"/>
    <line x1="${CX+12}" y1="${y_hcy-14}" x2="${CX+12}" y2="${y_hcy-20}" stroke="${hairC}" stroke-width="1.5" stroke-linecap="round"/>` : ''

  const faceGroup = `<g id="face_g">
    <ellipse cx="${CX-R+4}" cy="${y_hcy+6}" rx="8" ry="11" fill="${skin}"/>
    <ellipse cx="${CX+R-4}" cy="${y_hcy+6}" rx="8" ry="11" fill="${skin}"/>
    <circle cx="${CX}" cy="${y_hcy}" r="${R}" fill="${skin}"/>
    ${hairFront}
    <path d="M ${CX-21},${y_hcy-15} Q ${CX-11},${y_hcy-22} ${CX-3},${y_hcy-15}" fill="none" stroke="${hairC}" stroke-width="2.2" stroke-linecap="round"/>
    <path d="M ${CX+3},${y_hcy-15} Q ${CX+11},${y_hcy-22} ${CX+21},${y_hcy-15}" fill="none" stroke="${hairC}" stroke-width="2.2" stroke-linecap="round"/>
    <ellipse cx="${CX-13}" cy="${y_hcy-5}" rx="7" ry="8" fill="white"/>
    <circle cx="${CX-13}" cy="${y_hcy-4}" r="4.5" fill="${eyeC}"/>
    <circle cx="${CX-11}" cy="${y_hcy-6}" r="1.4" fill="white"/>
    <ellipse cx="${CX+13}" cy="${y_hcy-5}" rx="7" ry="8" fill="white"/>
    <circle cx="${CX+13}" cy="${y_hcy-4}" r="4.5" fill="${eyeC}"/>
    <circle cx="${CX+11}" cy="${y_hcy-6}" r="1.4" fill="white"/>
    ${lashes}
    <path d="M ${CX-4},${y_hcy+5} Q ${CX},${y_hcy+13} ${CX+4},${y_hcy+5}" fill="none" stroke="${lighten(skin,0.58)}" stroke-width="1.7" stroke-linecap="round"/>
    <path d="M ${CX-11},${y_hcy+18} Q ${CX},${y_hcy+27} ${CX+11},${y_hcy+18}" fill="none" stroke="${lipC}" stroke-width="2.4" stroke-linecap="round"/>
  </g>`

  // ── Body & dress path helpers ──────────────────────────
  function bodyP(sw:number,bw:number,ww:number,hw_:number,tw:number,cw:number,sh:number) {
    ...
    return `M ${L(sw)},${y_sh} C ${L(sw+8)},${y_sh+20} ${L(bw+5)},${y_bu-14} ${L(bw)},${y_bu} C ${L(bw-6)},${y_bu+22} ${L(ww+4)},${y_wa-12} ${L(ww)},${y_wa} C ${L(ww+5)},${y_wa+18} ${L(hw_-4)},${y_hi-10} ${L(hw_)},${y_hi} C ${L(hw_-2)},${y_hi+26} ${L(tw+4)},${y_th-9} ${L(tw)},${y_th} C ${L(tw-2)},${y_th+18} ${L(cw+2)},${y_kn-8} ${L(cw)},${y_kn} C ${L(cw)},${y_kn+22} ${L(cw-2)},${y_ca-6} ${L(cw-2)},${y_ca} C ${L(cw-2)},${y_ca+14} ${L(cw)},${y_ft-4} ${L(cw+2)},${y_ft} L ${R(cw+2)},${y_ft} C ${R(cw)},${y_ft-4} ${R(cw-2)},${y_ca+14} ${R(cw-2)},${y_ca} C ${R(cw-2)},${y_ca-6} ${R(cw)},${y_kn+22} ${R(cw)},${y_kn} C ${R(cw+2)},${y_kn-8} ${R(tw-2)},${y_th+18} ${R(tw)},${y_th} C ${R(tw+4)},${y_th-9} ${R(hw_-2)},${y_hi+26} ${R(hw_)},${y_hi} C ${R(hw_-4)},${y_hi-10} ${R(ww+5)},${y_wa+18} ${R(ww)},${y_wa} C ${R(ww+4)},${y_wa-12} ${R(bw-6)},${y_bu+22} ${R(bw)},${y_bu} C ${R(bw+5)},${y_bu-14} ${R(sw+8)},${y_sh+20} ${R(sw)},${y_sh} Z`
  }
  function dressP(sw:number,bw:number,ww:number,hw_:number,sh:number) {
    const sw2=sw+16, bw2=bw+10, ww2=ww+4, hw2=hw_+6
    const L=(v:number)=>CX-v+sh, R=(v:number)=>CX+v+sh
    return `M ${L(sw2)},${y_sh} C ${L(sw2+8)},${y_sh+20} ${L(bw2+5)},${y_bu-14} ${L(bw2)},${y_bu} C ${L(bw2-6)},${y_bu+22} ${L(ww2+4)},${y_wa-12} ${L(ww2)},${y_wa} C ${L(ww2+5)},${y_wa+18} ${L(hw2-4)},${y_hi-10} ${L(hw2)},${y_hi} C ${L(hw2+2)},${y_hi+28} ${L(hw2+4)},${y_ft-9} ${L(hw2-2)},${y_ft} L ${R(hw2-2)},${y_ft} C ${R(hw2+4)},${y_ft-9} ${R(hw2+2)},${y_hi+28} ${R(hw2)},${y_hi} C ${R(hw2-4)},${y_hi-10} ${R(ww2+5)},${y_wa+18} ${R(ww2)},${y_wa} C ${R(ww2+4)},${y_wa-12} ${R(bw2-6)},${y_bu+22} ${R(bw2)},${y_bu} C ${R(bw2+5)},${y_bu-14} ${R(sw2+8)},${y_sh+20} ${R(sw2)},${y_sh} Z`
  }
  function armP(s:number,sw:number,sh:number) {
    const ax=CX+s*sw+sh, ay=y_sh+8, ex=CX+s*(sw+26)+sh, ey=y_sh+94, hx=CX+s*(sw+10)+sh, hy=y_sh+186
    return `M ${ax},${ay} C ${ax+s*14},${ay+22} ${ex-s*5},${ey-20} ${ex},${ey} C ${ex+s*4},${ey+28} ${hx+s*8},${hy-28} ${hx},${hy}`
  }
  function neckP(nn:number,sh:number) {
    return `M ${CX-nn+sh},${y_nek+4} C ${CX-nn+2+sh},${y_nek+12} ${CX-nn+2+sh},${y_sh-7} ${CX-nn+3+sh},${y_sh} L ${CX+nn-3+sh},${y_sh} C ${CX+nn-2+sh},${y_sh-7} ${CX+nn-2+sh},${y_nek+12} ${CX+nn+sh},${y_nek+4} Z`
  }
  ...
  const dressImgW=(hi_w+28)*2, dressImgH=y_ft-y_sh+20

  const dressDefs = dressB64 ? `<clipPath id="dCl"><path id="dClP" d="${initDr}"/></clipPath>` : ''
  const dressLayer = dressB64
    ? `<image id="dImg" href="data:image/png;base64,${dressB64}" x="${CX-dressImgW/2}" y="${y_sh}" width="${dressImgW}" height="${dressImgH}" clip-path="url(#dCl)" preserveAspectRatio="xMidYMid slice" opacity="0.96"/>
       <path d="${initDr}" fill="none" stroke="rgba(0,0,0,0.07)" stroke-width="1.2"/>` : ''
  const sleeveLayer = dressB64
    ? `<path id="la2" d="${initLa}" fill="none" stroke="${skin_mid}" stroke-width="${Math.max(10,arm_w-4)}" stroke-linecap="round"/>
       <ellipse id="lh2" cx="${CX-sh_w-9}" cy="${y_sh+192}" rx="${ah}" ry="${ah+2}" fill="${skin}"/>
       <path id="ra2" d="${initRa}" fill="none" stroke="${skin_mid}" stroke-width="${Math.max(10,arm_w-4)}" stroke-linecap="round"/>
       <ellipse id="rh2" cx="${CX+sh_w+9}" cy="${y_sh+192}" rx="${ah}" ry="${ah+2}" fill="${skin}"/>` : ''

  // JavaScript for rotation — runs inside the iframe
  const js = `
...
var BT="${bodyType.replace(/"/g,'&quot;')}",hasDress=${dressB64?'true':'false'};
var DW=${dressImgW};
...
function SE(id,attr,val){var e=document.getElementById(id);if(e)e.setAttribute(attr,val);}
function SO(id,v){var e=document.getElementById(id);if(e)e.style.opacity=v;}
function bP(sw,bw,ww,hw,tw,cw,sh){
  var L=function(v){return CX-v+sh;},R=function(v){return CX+v+sh;};
  return'M '+L(sw)+','+YSH+' C '+L(sw+8)+','+(YSH+20)+' '+L(bw+5)+','+(YBU-14)+' '+L(bw)+','+YBU+' C '+L(bw-6)+','+(YBU+22)+' '+L(ww+4)+','+(YWA-12)+' '+L(ww)+','+YWA+' C '+L(ww+5)+','+(YWA+18)+' '+L(hw-4)+','+(YHI-10)+' '+L(hw)+','+YHI+' C '+L(hw-2)+','+(YHI+26)+' '+L(tw+4)+','+(YTH-9)+' '+L(tw)+','+YTH+' C '+L(tw-2)+','+(YTH+18)+' '+L(cw+2)+','+(YKN-8)+' '+L(cw)+','+YKN+' C '+L(cw)+','+(YKN+22)+' '+L(cw-2)+','+(YCA-6)+' '+L(cw-2)+','+YCA+' C '+L(cw-2)+','+(YCA+14)+' '+L(cw)+','+(YFT-4)+' '+L(cw+2)+','+YFT+' L '+R(cw+2)+','+YFT+' C '+R(cw)+','+(YFT-4)+' '+R(cw-2)+','+(YCA+14)+' '+R(cw-2)+','+YCA+' C '+R(cw-2)+','+(YCA-6)+' '+R(cw)+','+(YKN+22)+' '+R(cw)+','+YKN+' C '+R(cw+2)+','+(YKN-8)+' '+R(tw-2)+','+(YTH+18)+' '+R(tw)+','+YTH+' C '+R(tw+4)+','+(YTH-9)+' '+R(hw-2)+','+(YHI+26)+' '+R(hw)+','+YHI+' C '+R(hw-4)+','+(YHI-10)+' '+R(ww+5)+','+(YWA+18)+' '+R(ww)+','+YWA+' C '+R(ww+4)+','+(YWA-12)+' '+R(bw-6)+','+(YBU+22)+' '+R(bw)+','+YBU+' C '+R(bw+5)+','+(YBU-14)+' '+R(sw+8)+','+(YSH+20)+' '+R(sw)+','+YSH+' Z';
}
function dP(sw,bw,ww,hw,sh){
  var s2=sw+16,b2=bw+10,w2=ww+4,h2=hw+6;
  var L=function(v){return CX-v+sh;},R=function(v){return CX+v+sh;};
  return'M '+L(s2)+','+YSH+' C '+L(s2+8)+','+(YSH+20)+' '+L(b2+5)+','+(YBU-14)+' '+L(b2)+','+YBU+' C '+L(b2-6)+','+(YBU+22)+' '+L(w2+4)+','+(YWA-12)+' '+L(w2)+','+YWA+' C '+L(w2+5)+','+(YWA+18)+' '+L(h2-4)+','+(YHI-10)+' '+L(h2)+','+YHI+' C '+L(h2+2)+','+(YHI+28)+' '+L(h2+4)+','+(YFT-9)+' '+L(h2-2)+','+YFT+' L '+R(h2-2)+','+YFT+' C '+R(h2+4)+','+(YFT-9)+' '+R(h2+2)+','+(YHI+28)+' '+R(h2)+','+YHI+' C '+R(h2-4)+','+(YHI-10)+' '+R(w2+5)+','+(YWA+18)+' '+R(w2)+','+YWA+' C '+R(w2+4)+','+(YWA-12)+' '+R(b2-6)+','+(YBU+22)+' '+R(b2)+','+YBU+' C '+R(b2+5)+','+(YBU-14)+' '+R(s2+8)+','+(YSH+20)+' '+R(s2)+','+YSH+' Z';
}
function aP(s,sw,sh){
  var ax=CX+s*sw+sh,ay=YSH+8,ex=CX+s*(sw+26)+sh,ey=YSH+94,hx=CX+s*(sw+10)+sh,hy=YSH+186;
  return'M '+ax+','+ay+' C '+(ax+s*14)+','+(ay+22)+' '+(ex-s*5)+','+(ey-20)+' '+ex+','+ey+' C '+(ex+s*4)+','+(ey+28)+' '+(hx+s*8)+','+(hy-28)+' '+hx+','+hy;
}
function nP(nn,sh){
  return'M '+(CX-nn+sh)+','+(YNER+4)+' C '+(CX-nn+2+sh)+','+(YNER+12)+' '+(CX-nn+2+sh)+','+(YSH-7)+' '+(CX-nn+3+sh)+','+YSH+' L '+(CX+nn-3+sh)+','+YSH+' C '+(CX+nn-2+sh)+','+(YSH-7)+' '+(CX+nn-2+sh)+','+(YNER+12)+' '+(CX+nn+sh)+','+(YNER+4)+' Z';
}
function upd(a){
  a=m360(a);var r=a*Math.PI/180,cosA=Math.cos(r),sinA=Math.sin(r);
  var wS=Math.abs(cosA)*0.84+0.16,sh=Math.round(sinA*20);
  ...
  var nn=Math.max(5,Math.round(NW*wS));
  SE('body','d',bP(sw,bw,ww,hw,tw,cw,sh));
  SE('neck','d',nP(nn,sh));
  var fg=document.getElementById('face_g');
  if(fg){fg.setAttribute('transform','translate('+sh+',0)');fg.style.opacity=Math.max(0,cosA).toFixed(2);}
  SE('lft','cx',CX-cw+4+sh);SE('rft','cx',CX+cw-4+sh);
  if(!hasDress){
    var sL=!(a>28&&a<152),sR=!(a>208&&a<332);
    SE('la','d',aP(-1,sw,sh));SE('ra','d',aP(1,sw,sh));
    SO('la',sL?'1':'0');SO('lh',sL?'1':'0');SO('ra',sR?'1':'0');SO('rh',sR?'1':'0');
    SE('lh','cx',CX-sw-9+sh);SE('rh','cx',CX+sw+9+sh);
  }
  if(hasDress){
    var dc=document.getElementById('dClP');if(dc)dc.setAttribute('d',dP(sw,bw,ww,hw,sh));
    var di=document.getElementById('dImg');
    if(di){var sW=DW*wS;di.setAttribute('width',sW.toFixed(1));di.setAttribute('x',(CX-sW/2+sh).toFixed(1));di.style.opacity=(0.52+Math.max(0,cosA)*0.44).toFixed(2);}
    SE('la2','d',aP(-1,sw,sh));SE('ra2','d',aP(1,sw,sh));
    var sL2=!(a>28&&a<152),sR2=!(a>208&&a<332);
    SO('la2',sL2?'0.9':'0');SO('lh2',sL2?'0.9':'0');SO('ra2',sR2?'0.9':'0');SO('rh2',sR2?'0.9':'0');
    SE('lh2','cx',CX-sw-9+sh);SE('rh2','cx',CX+sw+9+sh);
  }
  SE('vl','x',CX+sh);
...
function snapTo(t){var st=angle,df=m360(t-st);if(df>180)df-=360;var steps=28,step=0;
...
function toggleSpin(){spinning=!spinning;var b=document.getElementById('sb');if(b)b.textContent=spinning?'\u23f8 Stop':'\u25b6 Spin';if(spinning)loop();}
...
if(sv){
  sv.addEventListener('mousedown',function(e){spinning=false;dragX=e.clientX;dragA=angle;sv.style.cursor='grabbing';e.preventDefault();});
...
html,body{width:100%;height:100%;background:#07071a;overflow:hidden;display:flex;flex-direction:column;align-items:center}
svg{cursor:grab;touch-action:none;width:100%;max-width:${W}px;height:auto;flex:1;min-height:0}
.ctrl{display:flex;gap:4px;flex-wrap:wrap;justify-content:center;padding:3px 4px;flex-shrink:0}
button{background:#2a1f60;color:#c8b8ff;border:1px solid #4a3898;padding:4px 10px;border-radius:7px;cursor:pointer;font-size:11px;font-weight:700}
input[type=range]{width:220px;accent-color:#8060e0;flex-shrink:0;margin-bottom:4px}
</style>
</head><body>
<svg id="av" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}">
<defs>
<radialGradient id="bgG" cx="50%" cy="48%" r="65%"><stop offset="0%" stop-color="#16123a"/><stop offset="100%" stop-color="#06061a"/></radialGradient>
...
<filter id="bl"><feGaussianBlur stdDeviation="2.5"/></filter>
<filter id="ds"><feDropShadow dx="1" dy="3" stdDeviation="4" flood-opacity="0.22"/></filter>
${dressDefs}
</defs>
<rect width="${W}" height="${H}" fill="url(#bgG)"/>
<ellipse cx="${CX}" cy="${y_ft+14}" rx="${hi_w+11}" ry="9" fill="rgba(0,0,0,0.15)" filter="url(#bl)"/>
<path id="la" d="${initLa}" fill="none" stroke="${skin_mid}" stroke-width="${arm_w}" stroke-linecap="round" opacity="${dressB64?'0':'1'}"/>
<ellipse id="lh" cx="${CX-sh_w-9}" cy="${y_sh+192}" rx="${ah}" ry="${ah+2}" fill="${skin}" opacity="${dressB64?'0':'1'}"/>
<path id="ra" d="${initRa}" fill="none" stroke="${skin_mid}" stroke-width="${arm_w}" stroke-linecap="round" opacity="${dressB64?'0':'1'}"/>
<ellipse id="rh" cx="${CX+sh_w+9}" cy="${y_sh+192}" rx="${ah}" ry="${ah+2}" fill="${skin}" opacity="${dressB64?'0':'1'}"/>
<path id="body" d="${initBd}" fill="url(#bG)" filter="url(#ds)"/>
<path d="M ${CX},${y_sh+5} C ${CX},${y_bu-5} ${CX},${y_bu+12} ${CX},${y_wa}" fill="none" stroke="rgba(255,255,255,0.09)" stroke-width="8" stroke-linecap="round"/>
${dressLayer}
${sleeveLayer}
<path id="neck" d="${initNk}" fill="${skin_mid}" filter="url(#ds)"/>
${faceGroup}
<ellipse id="lft" cx="${CX-ca_w+4}" cy="${y_ft+5}" rx="${ca_w+4}" ry="6" fill="${lighten(skin,0.54)}"/>
<ellipse id="rft" cx="${CX+ca_w-4}" cy="${y_ft+5}" rx="${ca_w+4}" ry="6" fill="${lighten(skin,0.54)}"/>
<text id="vl" x="${CX}" y="${H-4}" text-anchor="middle" font-size="9" font-family="system-ui" fill="rgba(255,255,255,0.15)">FRONT · 0° · ${bodyType}</text>
</svg>
<div class="ctrl">
...
<button id="sb" onclick="toggleSpin()">▶ Spin</button>
</div>
<input type="range" min="0" max="359" value="0" step="1" oninput="setAngle(+this.value)" id="sl"/>
...
export default function Home() {
  ...
  // Unique key on dress+bodytype ensures iframe re-renders when either changes
  const avatarFrame = (id: string) => {
    const k = `${id}-${result?.body_type}-${result?.skin_tone}-${dressB64?dressB64.slice(-16):'bare'}`
    return (
      <div style={{background:'#07071a',borderRadius:16,overflow:'hidden',border:'1px solid #1e1848'}}>
        <iframe key={k} srcDoc={result ? buildAvatar(result, dressB64) : ''}
          style={{width:'100%',height:520,border:'none',display:'block'}}
          title="3D Avatar" sandbox="allow-scripts"/>
      </div>
    )
  }

  const tabBtn = (id:string, lbl:string) => (
    <button onClick={()=>setActiveTab(id as any)} style={{padding:'10px 18px',border:'none',cursor:'pointer',fontWeight:700,fontSize:13,background:'transparent',color:activeTab===id?'#e8c99a':'#4040a0',borderBottom:activeTab===id?'2px solid #e8c99a':'2px solid transparent',whiteSpace:'nowrap'}}>
      {lbl}
    </button>
  )
  ...
  return (
    <main style={{minHeight:'100vh',background:'#06061a',color:'#e8e0ff',fontFamily:'system-ui,sans-serif'}}>
      <div style={{background:'linear-gradient(135deg,#1a0938,#0d0628)',padding:'16px 24px',borderBottom:'1px solid #1e1848',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
        <div>
          <h1 style={{margin:0,fontSize:'1.4rem',fontWeight:800,color:'#e8c99a'}}>👗 3D Fashion Stylist Pro</h1>
          <p style={{margin:'2px 0 0',color:'#7060a0',fontSize:'0.74rem'}}>AI body analysis · 3D avatar · Virtual try-on · Smart recommendations · v28</p>
        </div>
        ...
        {step==='upload'&&(
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(290px,1fr))',gap:18}}>
            <div style={{background:'#10103a',border:'1px solid #2a2860',borderRadius:16,padding:22}}>
              ...
              <div style={{color:'#5050a0',fontSize:12,marginBottom:14}}>Stand straight, facing camera, full body head to toe for best results</div>
              ...
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14,background:'#0d0d2a',borderRadius:10,padding:'8px 12px',border:'1px solid #1e1848'}}>
                ...
                <span style={{color:'#2a2a60',fontSize:11,marginLeft:4}}>(optional — improves accuracy)</span>
              </div>
            ...
            <div style={{background:'#10103a',border:'1px solid #1e1848',borderRadius:16,padding:22}}>
              ...
              {[['📏','Shoulder, Bust, Waist, High Hip & Low Hip measurements'],['👗','Indian size recommendation (XS–4XL)'],['🎨','Personalised colour palette for your skin tone'],['👤','Full-body 3D avatar with cartoon face — drag to rotate 360°'],['🪄','Virtual try-on — see outfits fitted to YOUR body shape'],['🛍','Shopping links on Amazon.in & Flipkart with exact size']].map(([e,t])=>(
                <div key={t as string} style={{display:'flex',gap:12,alignItems:'flex-start',marginBottom:12}}>
                  <span style={{fontSize:18,flexShrink:0}}>{e}</span><span style={{color:'#7060a0',fontSize:13}}>{t}</span>
                </div>
              ))}
              <div style={{marginTop:14,padding:'12px 14px',background:'#0d0d22',border:'1px solid #1a1848',borderRadius:10,fontSize:12,color:'#4a4880'}}>
                💡 <b style={{color:'#6a6898'}}>Best results:</b> Full body in frame, standing straight, plain background, good lighting. Works with sarees, suits, and casual wear.
              </div>
            </div>
          </div>
        )}
            ...
            {activeTab==='avatar'&&(
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:18}}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:14}}>
                  {visImg&&<div style={{background:'#10103a',border:'1px solid #2a2860',borderRadius:12,padding:8}}>
                    <div style={{color:'#6050a0',fontSize:11,marginBottom:4,paddingLeft:4}}>📐 Measurement detection overlay</div>
                    <img src={visImg} alt="detection" style={{width:'100%',borderRadius:8,maxHeight:220,objectFit:'contain',background:'#000'}}/>
                  </div>}
                  <div style={{background:'#10103a',border:'1px solid #2a2860',borderRadius:14,padding:16}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                      <span style={{fontSize:22}}>{result.body_icon}</span>
                      <span style={{color:'#e8c99a',fontWeight:800,fontSize:15}}>{result.body_type}</span>
                      <span style={{background:'#2a1f60',color:'#e8c99a',padding:'2px 10px',borderRadius:6,fontSize:14,fontWeight:700}}>{result.size}</span>
                    </div>
                    {result.quality_warnings?.length>0&&(
                      <div style={{background:'rgba(255,165,0,.08)',border:'1px solid #ffaa0033',borderRadius:6,padding:'6px 10px',marginBottom:8}}>
                        <div style={{color:'#ffaa00',fontWeight:700,fontSize:11,marginBottom:3}}>📷 Accuracy tips:</div>
                        {result.quality_warnings.slice(0,2).map((w:string)=><div key={w} style={{color:'#cc8800',fontSize:11}}>• {w}</div>)}
                      </div>
                    )}
                    <div style={{color:'#404060',fontSize:10,marginBottom:4}}>📏 {result.height_source} · {result.method}</div>
                    <div style={{color:'#6050a0',fontSize:12,marginBottom:10}}>{result.body_desc}</div>
                    <div style={{color:'#3a3060',fontSize:10,marginBottom:8,textAlign:'center'}}>Estimated ±3cm · 2D photo (depth estimated)</div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:5}}>
                      {[['Shoulder',result.shoulder_cm],['Bust',result.bust_cm],['Waist',result.waist_cm],['High Hip',result.high_hip_cm],['Hip (Low)',result.hip_cm],['Height',result.height_cm],['Hollow→Hem',result.hollow_to_hem_cm],['Inseam',result.inseam_cm]].map(([k,v])=>(
                        <div key={k as string} style={{background:'#0d0d22',border:'1px solid #1a1848',borderRadius:8,padding:'7px 10px'}}>
                          <div style={{color:'#4a4870',fontSize:9,textTransform:'uppercase',letterSpacing:1}}>{k}</div>
                          <div style={{color:'#e8e0ff',fontWeight:700,fontSize:15}}>{v}<span style={{fontSize:10,color:'#4a4870',marginLeft:2}}>cm</span></div>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div style={{background:'#10103a',border:'1px solid #2a2860',borderRadius:14,padding:14}}>
                    ...
                    <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
                      {result.best_colors.slice(0,8).map((c:string)=>(
                        <span key={c} style={{display:'inline-flex',alignItems:'center',gap:4,background:'#1e1848',color:'#b0a0e0',border:'1px solid #2e2868',borderRadius:8,padding:'3px 9px',fontSize:11}}>
                          <span style={{width:8,height:8,borderRadius:'50%',background:COLOR_HEX[c]||'#888',display:'inline-block',border:'1px solid rgba(255,255,255,0.15)'}}/>{c}
                        </span>
                      ))}
                    </div>
