...
export default function Home() {
  ...
  const analyze = async (file: File) => {
    ...
    try {
      const form = new FormData(); form.append('file',file); form.append('category',category); if(userHeight) form.append('user_height',userHeight)
    ...
    <main style={{minHeight:'100vh',background:'#06061a',color:'#e8e0ff',fontFamily:'system-ui,sans-serif'}}>
      <div style={{background:'linear-gradient(135deg,#1a0938,#0d0628)',padding:'18px 24px',borderBottom:'1px solid #1e1848',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
        <div>
          ...
          <p style={{margin:'2px 0 0',color:'#7060a0',fontSize:'0.76rem'}}>AI body analysis · 3D avatar with legs · Virtual try-on · Smart recommendations · v27</p>
        </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(290px,1fr))',gap:18}}>
            <div style={{background:'#10103a',border:'1px solid #2a2860',borderRadius:16,padding:22}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14,background:'#0d0d2a',borderRadius:10,padding:'8px 12px',border:'1px solid #1e1848'}}>
                ...
                <span style={{color:'#2a2a60',fontSize:11,marginLeft:4}}>(optional but improves accuracy 25%)</span>
              </div>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:14}}>
                  ...
                  <div style={{background:'#10103a',border:'1px solid #2a2860',borderRadius:14,padding:16}}>
                    ...
                    {result.quality_warnings&&result.quality_warnings.length>0&&(
                      <div style={{background:'rgba(255,165,0,.10)',border:'1px solid #ffaa0044',borderRadius:6,padding:'6px 10px',marginBottom:8}}>
                    ...
                    {result.height_source&&<div style={{color:'#404060',fontSize:10,marginBottom:4}}>📏 {result.height_source} · {result.quality||'Good'} photo quality</div>}
                    <div style={{color:'#6050a0',fontSize:12,marginBottom:12}}>{result.body_desc}</div>
                    <div style={{color:'#3a3060',fontSize:10,marginBottom:5,textAlign:'center'}}>Estimated ±3cm · 2D photo depth not available</div>
