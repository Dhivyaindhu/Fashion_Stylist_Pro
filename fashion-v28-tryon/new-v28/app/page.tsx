...
const PRODUCTS: Record<string, any[]> = {
  ...
  Kids: [
    ...
    { name: "Ethnic Wear Set",      bodies: ["Petite"],   tones: ["all"], colors: ["Fuchsia","Marigold","Cobalt"],              amazon: "https://www.amazon.in/s?k=kids+ethnic+wear",     flipkart: "https://www.flipkart.com/search?q=kids+ethnic+wear",myntra: "https://www.myntra.com/kids-ethnic-wear?rawQuery=kids+ethnic" },
  ],
}
...
export default function FashionStylist() {
  ...
  // ── Virtual try-on via Replicate IDM-VTON + Claude stylist ──
  const doTryOn = async () => {
    if (!photo || !dressFile) return
    setTryOnLoading(true); setTryOnImage(null); setTryOnDescription('')
    setError('')
    try {
      const toBase64 = (f: File): Promise<string> => new Promise((res, rej) => {
        const r = new FileReader()
        r.onload = () => res(r.result as string)          // full data-URI
        r.onerror = rej
        r.readAsDataURL(f)
      })
      const toBase64Raw = (f: File): Promise<string> => new Promise((res, rej) => {
        const r = new FileReader()
        r.onload = () => res((r.result as string).split(',')[1])  // raw base64
        r.onerror = rej
        r.readAsDataURL(f)
      })

      const personDataURI = await toBase64(photo)
      const dressDataURI  = await toBase64(dressFile)
      const personB64Raw  = await toBase64Raw(photo)
      const dressB64Raw   = await toBase64Raw(dressFile)
      const personMime    = photo.type    || 'image/jpeg'
      const dressMime     = dressFile.type || 'image/jpeg'

      // ── STEP 1: IDM-VTON via Replicate for photo-realistic try-on ──
      // Calls our Next.js proxy so the API token stays server-side
      setTryOnLoadingMsg('Generating photo-realistic try-on...')
      const tryonResp = await fetch('/api/tryon', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          human_img: personDataURI,
          garm_img:  dressDataURI,
          garment_des: 'outfit',
      ...
      if (tryonData.error) {
        // Fallback: canvas composite if Replicate fails / no API key
        console.warn('Replicate error, using canvas fallback:', tryonData.error)
        const composite = await createCanvasFallback(photo, dressFile)
        setTryOnImage(composite)
        setError('⚠️ Photo-realistic try-on unavailable (add REPLICATE_API_TOKEN to .env). Showing canvas overlay instead.')
      } else {
        setTryOnImage(tryonData.output_url)
      }

      // ── STEP 2: Claude stylist description (runs in parallel) ──
      setTryOnLoadingMsg('Getting AI stylist analysis...')
      const claudeResp = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: [
              { type: 'image', source: { type: 'base64', media_type: personMime, data: personB64Raw } },
              { type: 'image', source: { type: 'base64', media_type: dressMime,  data: dressB64Raw  } },
              { type: 'text', text: `You are a professional fashion stylist. Analyze how this specific outfit would look on this person.

Write 2–3 short paragraphs covering:
• Fit at bust, waist and hips given their body proportions
• How the color/print interacts with their skin tone
• Overall styling impression (neckline, hemline, sleeve length)
• One styling tip to elevate the look

End with exactly one line starting "VERDICT: " summarizing the overall match.` }
            ]
          }]
        })
      })
      const claudeData = await claudeResp.json()
      if (claudeData.content?.[0]?.text) setTryOnDescription(claudeData.content[0].text)
  ...
  // ── Canvas fallback (used when Replicate token not set) ──────
  const createCanvasFallback = async (personFile: File, dressFile: File): Promise<string> => {
    return new Promise((resolve) => {
      ...
      const onBoth = () => {
        ...
        const torsoTop = pImg.height * 0.13
        const torsoH   = pImg.height * 0.66
        const torsoW   = pImg.width  * 0.68
        const torsoX   = (pImg.width - torsoW) / 2
        ctx.globalAlpha = 0.85
        ctx.drawImage(dImg, torsoX, torsoTop, torsoW, torsoH)
        ctx.globalAlpha = 1
        resolve(canvas.toDataURL('image/jpeg', 0.92))
      }
      pImg.onload = () => { loaded++; if (loaded === 2) onBoth() }
      dImg.onload = () => { loaded++; if (loaded === 2) onBoth() }
      pImg.src = URL.createObjectURL(personFile)
      dImg.src = URL.createObjectURL(dressFile)
    })
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A14', color: '#EAE4FF', fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      {/* ── HEADER ── */}
      <header style={{ background: 'linear-gradient(135deg, #1a0636 0%, #0d0422 100%)', borderBottom: '1px solid #2a1850', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 900, background: 'linear-gradient(90deg, #e8c99a, #c97ef0)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            ✦ StyleSense AI
          </h1>
          <p style={{ margin: '2px 0 0', color: '#7060a0', fontSize: '0.72rem', letterSpacing: '0.08em' }}>
            AI BODY ANALYSIS · SKIN TONE · VIRTUAL TRY-ON · SMART SHOPPING
          </p>
        </div>
        {result && (
      ...
      <div style={{ background: '#0d0a24', borderBottom: '1px solid #1e1848', padding: '12px 24px', overflowX: 'auto' }}>
        <div style={{ display: 'flex', gap: 0, minWidth: 'max-content', margin: '0 auto', maxWidth: 900, justifyContent: 'center' }}>
          {STEPS.map((s, i) => (
            <button
              key={s}
              onClick={() => result && i > 0 && setStep(i)}
              disabled={i > 0 && !result}
              style={{
                display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', border: 'none',
                background: 'transparent', cursor: i === 0 || result ? 'pointer' : 'default',
                color: i === step ? '#e8c99a' : i < step ? '#8060c0' : '#3a3060',
                fontWeight: i === step ? 800 : 500, fontSize: 12,
                borderBottom: i === step ? '2px solid #e8c99a' : '2px solid transparent',
            ...
            >
              <span style={{
                ...
                color: i === step ? '#0a0a1a' : i < step ? '#c0a0ff' : '#4a4070',
                flexShrink: 0
              }}>{i < step ? '✓' : i + 1}</span>
              {s}
            </button>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 16px' }}>
        ...
        {step === 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
            <div style={{ background: '#12103a', border: '1px solid #2a2060', borderRadius: 18, padding: 24 }}>
              ...
              <p style={{ margin: '0 0 16px', color: '#5050a0', fontSize: 12 }}>Stand straight, face camera, full body head to toe visible</p>

              {/* Category */}
              <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                {['Women', 'Men', 'Kids'].map(c => (
                  <button key={c} onClick={() => setCategory(c)}
                    style={{ flex: 1, padding: '9px 0', border: `1.5px solid ${category === c ? '#8060e0' : '#1e1848'}`, borderRadius: 9, cursor: 'pointer', fontWeight: 700, fontSize: 12, background: category === c ? '#2a1f60' : '#0d0d2a', color: category === c ? '#e8c99a' : '#5040a0', transition: 'all 0.2s' }}>
                    {c}
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, background: '#0d0d2a', borderRadius: 10, padding: '8px 12px', border: '1px solid #1e1848' }}>
                ...
                <input type="number" value={userHeight} onChange={e => setUserHeight(e.target.value)} placeholder="e.g. 162" min="80" max="220"
                  style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', color: '#e8e0ff', fontSize: 14, fontWeight: 700, width: '60px' }} />
                <span style={{ color: '#4040a0', fontSize: 12 }}>cm (optional)</span>
              </div>

              {/* Upload area */}
              <div onClick={() => photoRef.current?.click()}
                style={{ border: '2px dashed #2a2860', borderRadius: 14, padding: 24, cursor: 'pointer', background: '#0d0d2a', textAlign: 'center', minHeight: 200, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, transition: 'border-color 0.2s' }}
              ...
              <input ref={photoRef} type="file" accept="image/*" style={{ display: 'none' }}
                onChange={e => { const f = e.target.files?.[0]; if (f) { setPhoto(f); setPhotoPreview(URL.createObjectURL(f)); analyze(f) } }} />
            ...
            <div style={{ background: '#12103a', border: '1px solid #1e1848', borderRadius: 18, padding: 24 }}>
              ...
              {[
                ['📐', 'Body Measurements', 'Shoulder, bust, waist, high hip, low hip, arm length — extracted via AI pose detection'],
                ['📏', 'Size Classification', 'Precise size from XS to XXXL using Indian standard measurement charts'],
                ['👤', 'Body Type Analysis', 'Identifies your type from 16 distinct body shapes for perfect styling guidance'],
                ['🎨', 'Skin Tone Detection', 'Fair/Light/Medium/Tan/Deep/Olive — with full color palette analysis'],
                ['✅', 'Color Recommendations', 'Colors that enhance your skin tone + what to avoid'],
                ['👗', 'Dress Recommendations', 'Outfits matched to your body type, size & skin tone'],
                ['🪄', 'Virtual Try-On', '2D preview: upload any outfit and see a styled description + visual composite'],
                ['🛒', 'Smart Shopping Links', 'Curated links on Amazon, Flipkart & Myntra with your exact size'],
              ].map(([icon, title, desc]) => (
              ...
              <div style={{ marginTop: 4, padding: '10px 12px', background: '#0d0d22', border: '1px solid #1a1848', borderRadius: 10, fontSize: 11, color: '#4a4880' }}>
                💡 <b style={{ color: '#6a6898' }}>Best photo:</b> Full body in frame, standing straight, arms slightly away from body, plain background
              </div>
            </div>
          </div>
        )}
        ...
        {step === 1 && result && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
            <div>
              <SectionHeader icon="📐" title="Body Measurements" subtitle={`Estimated from your photo · Category: ${category}`} />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[
                  ['Shoulder', result.shoulder_cm, '↔'],
                  ['Bust / Chest', result.bust_cm, '⬤'],
                  ['Waist', result.waist_cm, '◯'],
                  ['High Hip', result.high_hip_cm, '◐'],
                  ['Low Hip', result.hip_cm, '⬤'],
                  ['Height', result.height_cm, '↕'],
                  ['Hollow → Hem', result.hollow_to_hem_cm, '↓'],
                  ['Arm Length', result.arm_length_cm ?? Math.round((result.shoulder_cm || 40) * 1.6), '💪'],
                ].map(([k, v, ic]) => (
                  <MeasurementCard key={k as string} icon={ic as string} label={k as string} value={v} />
                ))}
              </div>
              <div style={{ marginTop: 12, padding: '10px 12px', background: '#0d0d22', border: '1px solid #1a1848', borderRadius: 10, fontSize: 11, color: '#4a4880' }}>
                ⚠️ Estimated ±3cm. 2D photo depth estimation has inherent variance. Provide height for better accuracy.
              </div>
            </div>
            <div>
              <SectionHeader icon="👕" title="Size Classification" subtitle="Based on Indian standard size chart" />
              <SizeCard result={result} category={category} />
              {result.quality_warnings?.length > 0 && (
                <div style={{ marginTop: 14, padding: '12px 14px', background: 'rgba(255,165,0,.08)', border: '1px solid #ffaa0033', borderRadius: 12 }}>
                  <div style={{ color: '#ffaa00', fontWeight: 700, fontSize: 12, marginBottom: 6 }}>📷 Tips for better accuracy:</div>
                  {result.quality_warnings.slice(0, 3).map((w: string) => (
                    <div key={w} style={{ color: '#cc8800', fontSize: 11, marginBottom: 3 }}>• {w}</div>
                  ))}
                </div>
              )}
              {photoPreview && (
                <div style={{ marginTop: 14 }}>
                  <div style={{ color: '#5050a0', fontSize: 11, marginBottom: 8, fontWeight: 700 }}>YOUR PHOTO</div>
                  <img src={photoPreview} alt="Your photo" style={{ width: '100%', maxHeight: 280, objectFit: 'contain', borderRadius: 12, border: '1px solid #2a2060' }} />
                </div>
              )}
            </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {bodyInfo?.tips.map((t: string) => (
                      <span key={t} style={{ background: '#2a1f60', color: '#c0a0ff', border: '1px solid #3a2880', borderRadius: 8, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>✓ {t}</span>
                    ))}
                  </div>
                </div>
              </div>
            </div>
            {/* All 16 body types grid */}
            <div style={{ color: '#5050a0', fontSize: 12, fontWeight: 700, marginBottom: 12, letterSpacing: '0.08em' }}>ALL 16 BODY TYPES</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 10 }}>
              {Object.entries(BODY_TYPES).map(([name, info]) => (
                <div key={name} style={{
                  background: name === result.body_type ? 'linear-gradient(135deg, #2a1860, #1a0d48)' : '#12103a',
                  border: `1.5px solid ${name === result.body_type ? '#6040c0' : '#1e1848'}`,
                  borderRadius: 12, padding: '12px 14px',
                  transition: 'all 0.2s'
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    <span style={{ fontSize: 22 }}>{info.icon}</span>
                    <span style={{ color: name === result.body_type ? '#e8c99a' : '#9080c0', fontWeight: 700, fontSize: 12 }}>
                      {name} {name === result.body_type && '← You'}
                    </span>
                  </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
            <div>
              ...
              <div style={{ background: 'linear-gradient(135deg, #1e1060, #120840)', border: '2px solid #4a2090', borderRadius: 18, padding: 20, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 16 }}>
                <div style={{ width: 70, height: 70, borderRadius: '50%', background: skinRec.hex, border: '3px solid #2a1860', boxShadow: `0 0 20px ${skinRec.hex}55`, flexShrink: 0 }} />
                <div>
                  <div style={{ fontSize: 20, fontWeight: 900, color: '#e8c99a' }}>{skinRec.label} Skin Tone</div>
                  <div style={{ color: '#9070c0', fontSize: 12, marginTop: 4 }}>
                    {skinRec.best.length} recommended colors · {skinRec.avoid.length} colors to avoid
                  </div>
                </div>
              </div>
              ...
              <div style={{ background: '#12103a', border: '1px solid #1e1848', borderRadius: 14, padding: 16, marginBottom: 14 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {skinRec.best.map((c: string) => (
                    <ColorChip key={c} name={c} hex={COLOR_HEX[c]} good />
                  ))}
                </div>
              </div>
              ...
              <div style={{ background: '#12103a', border: '1px solid #1e1848', borderRadius: 14, padding: 16 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {skinRec.avoid.map((c: string) => (
                    <ColorChip key={c} name={c} hex={COLOR_HEX[c] || '#888'} />
                  ))}
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              {result.style_tips && (
                <div style={{ background: '#12103a', border: '1px solid #1e1848', borderRadius: 14, padding: 16 }}>
                  ...
                  {(result.style_tips as string[]).map((t: string) => (
                    <div key={t} style={{ color: '#9080b0', fontSize: 12, marginBottom: 6, paddingLeft: 12, borderLeft: '2px solid #3a2070' }}>• {t}</div>
                  ))}
                </div>
              )}
            </div>
        ...
        {step === 4 && result && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
              {getRecommendedProducts().map((p: any) => (
                <ProductCard key={p.name} product={p} bestColors={new Set(skinRec?.best || [])} size={result.size} />
              ))}
            </div>
        ...
        {step === 5 && result && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(310px, 1fr))', gap: 20 }}>
            {/* ── Left: Controls ── */}
            <div>
              <SectionHeader icon="🪄" title="Virtual Try-On" subtitle="Powered by IDM-VTON · Photo-realistic 2D result" />
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', background: 'linear-gradient(90deg,#1a0a3a,#0d0520)', border: '1px solid #4a2090', borderRadius: 12, padding: '10px 14px', marginBottom: 16 }}>
                <span style={{ fontSize: 20 }}>✨</span>
                <div style={{ fontSize: 11, lineHeight: 1.5 }}>
                  <span style={{ color: '#c0a0ff', fontWeight: 800 }}>IDM-VTON</span>
                  <span style={{ color: '#5050a0' }}> (Replicate) generates a photo-realistic try-on image. </span>
                  <span style={{ color: '#8060c0' }}>Claude AI adds a stylist analysis report.</span>
                </div>
              </div>
              {/* Person photo */}
              <div style={{ background: '#12103a', border: '1px solid #2a2060', borderRadius: 16, padding: 16, marginBottom: 14 }}>
                <div style={{ color: '#e8c99a', fontWeight: 800, marginBottom: 8, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ background: '#4a2090', color: '#e8c99a', width: 20, height: 20, borderRadius: '50%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 900 }}>1</span>
              ...
              <div style={{ background: '#12103a', border: '1px solid #2a2060', borderRadius: 16, padding: 16, marginBottom: 14 }}>
                <div style={{ color: '#e8c99a', fontWeight: 800, marginBottom: 10, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ background: '#4a2090', color: '#e8c99a', width: 20, height: 20, borderRadius: '50%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 900 }}>2</span>
                  Garment Type
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  {([['upper_body','👕 Top'],['lower_body','👖 Bottom'],['dresses','👗 Dress/Full']] as const).map(([val, lbl]) => (
                    <button key={val} onClick={() => setTryOnCategory(val)}
                      style={{ flex: 1, padding: '8px 4px', border: `1.5px solid ${tryOnCategory === val ? '#8060e0' : '#1e1848'}`, borderRadius: 9, cursor: 'pointer', fontWeight: 700, fontSize: 11, background: tryOnCategory === val ? '#2a1f60' : '#0d0d2a', color: tryOnCategory === val ? '#e8c99a' : '#5040a0', transition: 'all 0.2s' }}>
                      {lbl}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ background: '#12103a', border: '1px solid #2a2060', borderRadius: 16, padding: 16, marginBottom: 14 }}>
                <div style={{ color: '#e8c99a', fontWeight: 800, marginBottom: 8, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ background: '#4a2090', color: '#e8c99a', width: 20, height: 20, borderRadius: '50%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 900 }}>3</span>
                  Upload Outfit Image {dressPreview && <span style={{ marginLeft: 'auto', color: '#22c55e', fontSize: 11 }}>✓ Ready</span>}
                </div>
                <div onClick={() => dressRef.current?.click()}
                  style={{ border: '2px dashed #2a2860', borderRadius: 12, padding: 16, cursor: 'pointer', background: '#0d0d2a', textAlign: 'center', minHeight: 130, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}
                  onMouseEnter={e => (e.currentTarget.style.borderColor = '#6040c0')}
                  onMouseLeave={e => (e.currentTarget.style.borderColor = '#2a2860')}>
                  {dressPreview
                    ? <img src={dressPreview} alt="outfit" style={{ maxHeight: 130, maxWidth: '100%', borderRadius: 8, objectFit: 'contain' }} />
                    : <><div style={{ fontSize: 30 }}>👗</div><div style={{ color: '#4040a0', fontSize: 12 }}>Click to upload outfit</div><div style={{ color: '#2a2a60', fontSize: 10 }}>Best: white/plain background · full garment visible</div></>}
                </div>
                <input ref={dressRef} type="file" accept="image/*" style={{ display: 'none' }}
                  onChange={e => { const f = e.target.files?.[0]; if (f) { setDressFile(f); setDressPreview(URL.createObjectURL(f)); setTryOnImage(null); setTryOnDescription('') } }} />
                {dressFile && <button onClick={() => { setDressFile(null); setDressPreview(null); setTryOnImage(null); setTryOnDescription('') }}
                  style={{ marginTop: 8, width: '100%', background: '#1a0a20', color: '#c060a0', border: '1px solid #401030', padding: '6px', borderRadius: 8, cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>🗑️ Remove</button>}
              </div>
              {/* Generate button */}
              <button onClick={doTryOn} disabled={!photo || !dressFile || tryOnLoading}
                style={{ width: '100%', padding: '14px', borderRadius: 12, border: 'none', cursor: (!photo || !dressFile || tryOnLoading) ? 'not-allowed' : 'pointer',
                  background: (!photo || !dressFile || tryOnLoading) ? '#1a1848' : 'linear-gradient(135deg,#5030b0,#8050d0)',
                  color: (!photo || !dressFile || tryOnLoading) ? '#4040a0' : '#fff', fontWeight: 900, fontSize: 15, transition: 'all 0.2s',
                  boxShadow: (!photo || !dressFile || tryOnLoading) ? 'none' : '0 4px 20px rgba(128,80,210,0.4)' }}>
                {tryOnLoading ? `⏳ ${tryOnLoadingMsg || 'Processing...'}` : '✨ Generate Photo-Realistic Try-On'}
              </button>
              <div style={{ marginTop: 10, padding: '10px 12px', background: '#0d0d22', border: '1px solid #1a1848', borderRadius: 10, fontSize: 11, color: '#4a4880', lineHeight: 1.6 }}>
                <b style={{ color: '#6a6898' }}>Requires:</b> <code style={{ color: '#8060c0' }}>REPLICATE_API_TOKEN</code> in <code style={{ color: '#8060c0' }}>.env.local</code><br/>
                Without it a canvas overlay fallback is used. Get token at replicate.com · ~$0.05/generation.
              </div>
            </div>
            {/* ── Right: Result ── */}
            <div>
              {tryOnLoading && (
                <div style={{ background: '#12103a', border: '1px solid #2a2060', borderRadius: 16, padding: 32, textAlign: 'center' }}>
                  <div style={{ fontSize: 48, marginBottom: 14 }}>✨</div>
                  <div style={{ color: '#c0a0ff', fontSize: 15, fontWeight: 800, marginBottom: 6 }}>{tryOnLoadingMsg || 'Generating...'}</div>
                  <div style={{ color: '#5050a0', fontSize: 12, marginBottom: 20 }}>IDM-VTON is warping the garment to your body shape</div>
                  {['Uploading images to IDM-VTON', 'AI warping garment to body', 'Getting Claude stylist analysis'].map((stage, i) => (
                    <div key={stage} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 12px', background: '#0d0d22', borderRadius: 8, marginBottom: 6, textAlign: 'left' }}>
                      ...
                      <span style={{ color: '#6060a0', fontSize: 11 }}>{stage}</span>
                    </div>
                  ))}
              ...
              {tryOnImage && !tryOnLoading && (
                <div style={{ background: '#12103a', border: '1px solid #2a2060', borderRadius: 16, overflow: 'hidden', marginBottom: 14 }}>
                  <div style={{ padding: '12px 16px', borderBottom: '1px solid #1e1848', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ color: '#e8c99a', fontWeight: 800, fontSize: 13 }}>📸 Photo-Realistic Try-On</span>
                    ...
                    <a href={tryOnImage} download="tryon-result.jpg" target="_blank" rel="noreferrer"
              ...
              {tryOnDescription && !tryOnLoading && (
                <div style={{ background: '#12103a', border: '1px solid #2a2060', borderRadius: 16, padding: 18 }}>
                  <div style={{ color: '#e8c99a', fontWeight: 800, fontSize: 14, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span>🪞</span> AI Stylist Report
                    <span style={{ marginLeft: 'auto', background: '#1e1048', color: '#8060c0', fontSize: 10, padding: '2px 8px', borderRadius: 6 }}>Claude AI</span>
                  </div>
                  {tryOnDescription.split('\n').filter(Boolean).map((para, i) => {
                    const isVerdict = para.startsWith('VERDICT:')
                    return (
                      <div key={i} style={{ marginBottom: 10, fontSize: 13, lineHeight: 1.7, color: isVerdict ? '#22c55e' : '#9080c0',
                        background: isVerdict ? 'rgba(34,197,94,0.08)' : 'transparent', borderLeft: isVerdict ? '3px solid #22c55e' : 'none',
                        padding: isVerdict ? '8px 12px' : '0', borderRadius: isVerdict ? 8 : 0, fontWeight: isVerdict ? 800 : 400 }}>
                        {para}
                      </div>
                    )
                  })}
              ...
              {!tryOnImage && !tryOnLoading && !tryOnDescription && (
                <div style={{ background: '#12103a', border: '1px solid #1e1848', borderRadius: 16, padding: 40, textAlign: 'center' }}>
                  ...
                  <div style={{ color: '#4040a0', fontSize: 14, marginBottom: 6 }}>Select outfit type, upload garment & click Generate</div>
                  <div style={{ color: '#2a2a60', fontSize: 11 }}>IDM-VTON will render your exact body wearing the outfit</div>
                </div>
              )}
            </div>
...
function SectionHeader({ icon, title, subtitle }: { icon: string; title: string; subtitle: string }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <h2 style={{ margin: '0 0 4px', fontSize: 18, fontWeight: 900, color: '#e8e0ff', display: 'flex', alignItems: 'center', gap: 8 }}>
...
function MeasurementCard({ icon, label, value }: { icon: string; label: string; value: any }) {
  return (
    <div style={{ background: '#0d0d22', border: '1px solid #1a1848', borderRadius: 10, padding: '10px 12px' }}>
      ...
      <div style={{ color: '#e8e0ff', fontWeight: 800, fontSize: 18 }}>
        {value ?? '–'}
        <span style={{ fontSize: 10, color: '#4a4870', marginLeft: 2, fontWeight: 400 }}>cm</span>
      </div>
    </div>
  )
}
    ...
    <div style={{ background: '#12103a', border: '1px solid #2a2060', borderRadius: 14, overflow: 'hidden' }}>
      <div style={{ background: 'linear-gradient(135deg, #2a1060, #1a0848)', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          ...
          <div style={{ color: '#6050a0', fontSize: 11 }}>Based on bust {result.bust_cm}cm · {category}</div>
        </div>
      </div>
      <div style={{ padding: 14 }}>
        <div style={{ color: '#4a4870', fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', marginBottom: 8 }}>FULL SIZE CHART — BUST (cm)</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
          {Object.entries(chart).map(([size, range]) => (
            <div key={size} style={{
              padding: '4px 9px', borderRadius: 7, fontSize: 11, fontWeight: 700,
              background: size === result.size ? '#4a2090' : '#0d0d22',
              border: `1px solid ${size === result.size ? '#8060e0' : '#1a1848'}`,
              color: size === result.size ? '#e8c99a' : '#4a4870'
            }}>
...
function ColorChip({ name, hex, good }: { name: string; hex: string; good?: boolean }) {
  return (
    <span style={{
      ...
      background: '#1e1848', color: good ? '#c0b0e0' : '#707090',
      border: `1px solid ${good ? '#2e2868' : '#1a1840'}`,
      borderRadius: 8, padding: '3px 9px', fontSize: 11, fontWeight: good ? 700 : 400
    }}>
      <span style={{ width: 8, height: 8, borderRadius: '50%', background: hex, border: '1px solid rgba(255,255,255,0.15)', display: 'inline-block', flexShrink: 0 }} />
      {name}
    </span>
  )
}
...
function ProductCard({ product, bestColors, size }: { product: any; bestColors: Set<string>; size: string }) {
  ...
  return (
    <div style={{ background: '#12103a', border: '1px solid #1e1848', borderRadius: 16, padding: 18, display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ color: '#3a3070', fontSize: 11 }}>
        Best for: <span style={{ color: '#5050a0' }}>{product.bodies.slice(0, 2).join(', ')}</span>
      </div>
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        <a href={`${product.amazon}&rh=p_n_size_browse-vebin:${encodeURIComponent(size)}`} target="_blank" rel="noreferrer"
          style={{ background: '#ff9900', color: '#000', padding: '7px 12px', borderRadius: 8, fontWeight: 800, fontSize: 11, textDecoration: 'none', flex: 1, textAlign: 'center', minWidth: 80 }}>
          🛒 Amazon
        </a>
        <a href={`${product.flipkart}&p%5B%5D=facets.size%255B%255D%3D${encodeURIComponent(size)}`} target="_blank" rel="noreferrer"
          style={{ background: '#2874f0', color: '#fff', padding: '7px 12px', borderRadius: 8, fontWeight: 800, fontSize: 11, textDecoration: 'none', flex: 1, textAlign: 'center', minWidth: 80 }}>
          🛒 Flipkart
        </a>
        <a href={product.myntra} target="_blank" rel="noreferrer"
          style={{ background: '#ff3f6c', color: '#fff', padding: '7px 12px', borderRadius: 8, fontWeight: 800, fontSize: 11, textDecoration: 'none', flex: 1, textAlign: 'center', minWidth: 80 }}>
          🛒 Myntra
        </a>
      </div>
    </div>
...
function NavButtons({ onPrev, onNext, nextLabel = 'Next →', showNext = true }: { onPrev?: () => void; onNext?: () => void; nextLabel?: string; showNext?: boolean }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 20, paddingTop: 16, borderTop: '1px solid #1e1848', gridColumn: '1 / -1' }}>
      {onPrev
        ? <button onClick={onPrev} style={{ padding: '10px 20px', borderRadius: 10, border: '1px solid #2a2060', background: '#12103a', color: '#9070c0', cursor: 'pointer', fontWeight: 700, fontSize: 13 }}>← Back</button>
        : <div />
      }
      {showNext && onNext && (
        <button onClick={onNext} style={{ padding: '10px 22px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #4a2090, #7040c0)', color: '#fff', cursor: 'pointer', fontWeight: 800, fontSize: 13 }}>{nextLabel}</button>
      )}
    </div>
  )
}
