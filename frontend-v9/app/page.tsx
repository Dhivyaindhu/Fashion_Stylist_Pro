...
function buildAvatar(result: any, dressB64: string|null): string {
  ...
  const dressW=hi_w*2+20
  const dressH=y_bot-y_sh+20
  const dressDefs=dressB64
    ? `<clipPath id="bClip"><path id="bClipP" d="${initBd}"/></clipPath>`+
      `<pattern id="dP" patternUnits="userSpaceOnUse" x="${CX-dressW/2}" y="${y_sh}" width="${dressW}" height="${dressH}">`+
      `<image href="data:image/png;base64,${dressB64}" x="0" y="0" width="${dressW}" height="${dressH}" preserveAspectRatio="xMidYMid slice"/>`+
      `</pattern>`
    : ''
  const dressBody=dressB64
    ? `<path id="dPth" d="${initBd}" fill="url(#dP)" clip-path="url(#bClip)" opacity="0.94"/>`+
      `<path d="${initBd}" fill="none" stroke="rgba(0,0,0,0.15)" stroke-width="2"/>`
    : ''
...
function upd(a){
  ...
  if(hasDress){
    // Update dress pattern position and clip path together
    var dp=document.getElementById('dPth'); var dc=document.getElementById('bClipP');
    if(dp) dp.setAttribute('d',bd);
    if(dc) dc.setAttribute('d',bd);
    // Shift the pattern with the body
    var pat=document.getElementById('dP');
    if(pat) pat.setAttribute('x',CX-DW/2+sh);
  }
  S('head','cx',CX+sh);
