...
function buildAvatar(result: any, dressB64: string|null): string {
  ...
  const hw=(c:number)=>Math.max(12,Math.round((c/(2*Math.PI))*SC))
  const bu_w=hw(result.bust_cm??88)
  const wa_w=hw(result.waist_cm??72)
  const hi_w=hw(result.hip_cm??90)
  const sh_w=Math.max(hw((result.shoulder_cm??40)*1.08),58)
  const arm_w=Math.max(16,Math.round(sh_w*0.30))
  const nw=Math.max(14,Math.round(sh_w*0.28))
  const ah=Math.round(arm_w/2)

  const y_sh=215, y_bu=y_sh+70, y_wa=y_bu+60, y_hi=y_wa+42, y_bot=y_hi+85
  const y_nek=y_sh-22, y_hcy=y_nek-78

  const skin_sh=lighten(skin,0.70)
  const skin_hi=lighten(skin,1.30)
  const skin_mid=lighten(skin,0.86)

  const SKINS: Record<string,{ds:string,h:string}> = {
    Fair:{ds:'f8d5c2',h:'b8860b'},Light:{ds:'e8b89a',h:'4a3728'},
    Medium:{ds:'c68642',h:'2d1b0e'},Tan:{ds:'a0522d',h:'1a0f0a'},Deep:{ds:'4a2912',h:'0a0505'}
  }
  ...
  // Body path (torso only — used for both body fill and dress drape)
  const bodyD=(sw:number,bw:number,ww:number,hw_:number,sh=0)=>
    `M ${CX-sw+sh},${y_sh} C ${CX-sw-8+sh},${y_sh+22} ${CX-bw-5+sh},${y_bu-16} ${CX-bw+sh},${y_bu} `+
    `C ${CX-bw+6+sh},${y_bu+26} ${CX-ww-4+sh},${y_wa-14} ${CX-ww+sh},${y_wa} `+
    `C ${CX-ww-5+sh},${y_wa+20} ${CX-hw_+4+sh},${y_hi-12} ${CX-hw_+sh},${y_hi} `+
    `C ${CX-hw_+sh},${y_hi+30} ${CX-hw_+10+sh},${y_bot-8} ${CX-hw_+14+sh},${y_bot} `+
    `L ${CX+hw_-14+sh},${y_bot} `+
    `C ${CX+hw_-10+sh},${y_bot-8} ${CX+hw_+sh},${y_hi+30} ${CX+hw_+sh},${y_hi} `+
    `C ${CX+hw_-4+sh},${y_hi-12} ${CX+ww+5+sh},${y_wa+20} ${CX+ww+sh},${y_wa} `+
    `C ${CX+ww+4+sh},${y_wa-14} ${CX+bw-6+sh},${y_bu+26} ${CX+bw+sh},${y_bu} `+
    `C ${CX+bw+5+sh},${y_bu-16} ${CX+sw+8+sh},${y_sh+22} ${CX+sw+sh},${y_sh} Z`

  // Arm path
  const armD=(s:number,sw:number,sh=0)=>{
    const ax=CX+s*sw+sh, ay=y_sh+10
    const ex=CX+s*(sw+30)+sh, ey=y_sh+108
    const hx=CX+s*(sw+12)+sh, hy=y_sh+205
    return `M ${ax},${ay} C ${ax+s*18},${ay+28} ${ex-s*5},${ey-26} ${ex},${ey} C ${ex+s*4},${ey+34} ${hx+s*10},${hy-34} ${hx},${hy}`
  }
  const neckD=(nn:number,sh=0)=>
    `M ${CX-nn+sh},${y_nek+4} C ${CX-nn+2+sh},${y_nek+14} ${CX-nn+2+sh},${y_sh-8} ${CX-nn+3+sh},${y_sh} `+
    `L ${CX+nn-3+sh},${y_sh} C ${CX+nn-2+sh},${y_sh-8} ${CX+nn-2+sh},${y_nek+14} ${CX+nn+sh},${y_nek+4} Z`

  const initBd=bodyD(sh_w,bu_w,wa_w,hi_w)
  const initLa=armD(-1,sh_w), initRa=armD(1,sh_w)
  const initNk=neckD(nw)

  // Dress overlay — uses SAME body path, centered pattern
  // Dress: direct <image> clipped to body shape — simpler and always correct
  const dressW = (sh_w + 40) * 2        // full width including arms
  const dressH = y_bot - y_sh + 30
  const dressX = CX - dressW/2
  const dressDefs = dressB64
    ? `<clipPath id="bClip"><path id="bClipP" d="${initBd}"/></clipPath>`
    : ''
  const dressBody = dressB64
    ? `<image id="dImg" href="data:image/png;base64,${dressB64}" `+
      `x="${dressX}" y="${y_sh}" width="${dressW}" height="${dressH}" `+
      `clip-path="url(#bClip)" preserveAspectRatio="xMidYMid meet" opacity="0.95"/>`+
      `<path d="${initBd}" fill="none" stroke="rgba(0,0,0,0.12)" stroke-width="1.5"/>`
    : ''
  const armOp=dressB64?'0':'1'

  return `<!DOCTYPE html><html><head><meta charset="utf-8"/>
<style>*{margin:0;padding:0;box-sizing:border-box}body{background:#07071a;display:flex;justify-content:center;padding:10px;font-family:system-ui}svg{cursor:grab}</style>
</head><body>
<div style="display:flex;flex-direction:column;align-items:center;gap:10px;width:100%">
<svg id="av" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" style="max-width:100%">
<defs>
<radialGradient id="bgG" cx="50%" cy="55%" r="60%"><stop offset="0%" stop-color="#1a1535"/><stop offset="100%" stop-color="#07071a"/></radialGradient>
<linearGradient id="bG" x1="0%" y1="0%" x2="100%" y2="0%">
  <stop offset="0%" stop-color="${skin_sh}"/>
  <stop offset="35%" stop-color="${skin}"/>
  <stop offset="55%" stop-color="${skin_hi}" stop-opacity="0.85"/>
  <stop offset="100%" stop-color="${skin_sh}"/>
</linearGradient>
<filter id="bl"><feGaussianBlur stdDeviation="4"/></filter>
<filter id="ds"><feDropShadow dx="2" dy="4" stdDeviation="5" flood-opacity="0.28"/></filter>
${dressDefs}
</defs>
<rect width="${W}" height="${H}" fill="url(#bgG)"/>
<ellipse cx="${CX}" cy="${y_bot+16}" rx="${hi_w+8}" ry="10" fill="rgba(0,0,0,0.15)" filter="url(#bl)"/>
<path id="la" d="${initLa}" fill="none" stroke="${skin_mid}" stroke-width="${arm_w}" stroke-linecap="round" opacity="${armOp}"/>
<ellipse id="lh" cx="${CX-sh_w-12}" cy="${y_sh+213}" rx="${Math.max(9,ah+1)}" ry="${Math.max(11,ah+3)}" fill="${skin}" opacity="${armOp}"/>
<path id="ra" d="${initRa}" fill="none" stroke="${skin_mid}" stroke-width="${arm_w}" stroke-linecap="round" opacity="${armOp}"/>
<ellipse id="rh" cx="${CX+sh_w+12}" cy="${y_sh+213}" rx="${Math.max(9,ah+1)}" ry="${Math.max(11,ah+3)}" fill="${skin}" opacity="${armOp}"/>
<path id="torso" d="${initBd}" fill="url(#bG)" filter="url(#ds)"/>
<path d="M ${CX},${y_sh+6} C ${CX},${y_bu-8} ${CX},${y_bu+18} ${CX},${y_wa}" fill="none" stroke="rgba(255,255,255,0.14)" stroke-width="12" stroke-linecap="round"/>
${dressBody}
<path id="neck" d="${initNk}" fill="${skin_mid}" filter="url(#ds)"/>
<circle id="head" cx="${CX}" cy="${y_hcy}" r="70" fill="${skin}" filter="url(#ds)"/>
<image id="face" href="${dbUrl}" x="${CX-84}" y="${y_hcy-90}" width="168" height="168" clip-path="circle(70px at 84px 84px)" preserveAspectRatio="xMidYMid meet"/>
<text id="vl" x="${CX}" y="${H-6}" text-anchor="middle" font-size="11" font-family="system-ui" fill="rgba(255,255,255,0.22)">FRONT · 0° · ${bodyType}</text>
</svg>
<div style="display:flex;gap:6px;flex-wrap:wrap;justify-content:center">
  ...
  <button id="sb" onclick="toggleSpin()" style="background:#1a1040;color:#8070c0;border:1px solid #2e2060;padding:6px 12px;border-radius:8px;cursor:pointer;font-size:11px">▶ Spin</button>
</div>
<input type="range" min="0" max="359" value="0" step="1" style="width:280px;accent-color:#8060e0" oninput="setAngle(+this.value)" id="sl"/>
</div>
<script>
(function(){
var CX=${CX},SHW=${sh_w},BUW=${bu_w},WAW=${wa_w},HIW=${hi_w},NW=${nw},ARW=${arm_w};
var YSH=${y_sh},YBU=${y_bu},YWA=${y_wa},YHI=${y_hi},YBOT=${y_bot},YNER=${y_nek},YHCY=${y_hcy};
var BT="${bodyType}",hasDress=${dressB64?'true':'false'};
var DW=${dressW},DH=${dressH};
...
function upd(a){
  a=m360(a);var r=a*Math.PI/180,c=Math.cos(r),s=Math.sin(r);
  var wS=Math.abs(c)*0.84+0.16, sh=Math.round(s*20);
  ...
  var nn=Math.max(6,Math.round(NW*wS)),aw=Math.round(ARW*wS),ah=Math.round(aw/2);
  var bd='M '+(CX-sw+sh)+','+YSH+' C '+(CX-sw-8+sh)+','+(YSH+22)+' '+(CX-bw-5+sh)+','+(YBU-16)+' '+(CX-bw+sh)+','+YBU+
    ' C '+(CX-bw+6+sh)+','+(YBU+26)+' '+(CX-ww-4+sh)+','+(YWA-14)+' '+(CX-ww+sh)+','+YWA+
    ' C '+(CX-ww-5+sh)+','+(YWA+20)+' '+(CX-hw+4+sh)+','+(YHI-12)+' '+(CX-hw+sh)+','+YHI+
    ' C '+(CX-hw+sh)+','+(YHI+30)+' '+(CX-hw+10+sh)+','+(YBOT-8)+' '+(CX-hw+14+sh)+','+YBOT+
    ' L '+(CX+hw-14+sh)+','+YBOT+
    ' C '+(CX+hw-10+sh)+','+(YBOT-8)+' '+(CX+hw+sh)+','+(YHI+30)+' '+(CX+hw+sh)+','+YHI+
    ' C '+(CX+hw-4+sh)+','+(YHI-12)+' '+(CX+ww+5+sh)+','+(YWA+20)+' '+(CX+ww+sh)+','+YWA+
    ' C '+(CX+ww+4+sh)+','+(YWA-14)+' '+(CX+bw-6+sh)+','+(YBU+26)+' '+(CX+bw+sh)+','+YBU+
    ' C '+(CX+bw+5+sh)+','+(YBU-16)+' '+(CX+sw+8+sh)+','+(YSH+22)+' '+(CX+sw+sh)+','+YSH+' Z';
  function ap(side){
    var ax=CX+side*sw+sh,ay=YSH+10,ex=CX+side*(sw+30)+sh,ey=YSH+108,hx=CX+side*(sw+12)+sh,hy=YSH+205;
    return 'M '+ax+','+ay+' C '+(ax+side*18)+','+(ay+28)+' '+(ex-side*5)+','+(ey-26)+' '+ex+','+ey+
      ' C '+(ex+side*4)+','+(ey+34)+' '+(hx+side*10)+','+(hy-34)+' '+hx+','+hy;
  }
  var nd='M '+(CX-nn+sh)+','+(YNER+4)+' C '+(CX-nn+2+sh)+','+(YNER+14)+' '+(CX-nn+2+sh)+','+(YSH-8)+' '+(CX-nn+3+sh)+','+YSH+
    ' L '+(CX+nn-3+sh)+','+YSH+' C '+(CX+nn-2+sh)+','+(YSH-8)+' '+(CX+nn-2+sh)+','+(YNER+14)+' '+(CX+nn+sh)+','+(YNER+4)+' Z';

  S('torso','d',bd); S('neck','d',nd);
  S('head','cx',CX+sh);
  var fi=document.getElementById('face'); if(fi){fi.setAttribute('x',CX-84+sh);}
  O('face',Math.max(0,c).toFixed(2));

  if(!hasDress){
    var showL=!(a>25&&a<155), showR=!(a>205&&a<335);
    S('la','d',ap(-1)); S('ra','d',ap(1));
    O('la',showL?'1':'0'); O('lh',showL?'1':'0');
    O('ra',showR?'1':'0'); O('rh',showR?'1':'0');
  }
  if(hasDress){
    // Update clip path and dress image position together with body shift
    var dc=document.getElementById('bClipP'); if(dc) dc.setAttribute('d',bd);
    var di=document.getElementById('dImg');
    if(di){
      // Shift dress image to follow body horizontal offset
      var newX = CX - DW/2 + sh;
      di.setAttribute('x', newX);
      // Scale dress width with body width (perspective foreshortening)
      di.setAttribute('width', DW * wS);
      di.setAttribute('x', CX - (DW*wS)/2 + sh);
    }
  }
  S('head','cx',CX+sh);
...
function snapTo(t){var st=angle,df=m360(t-st);if(df>180)df-=360;var steps=30,step=0;
  function tick(){step++;var p=step/steps;p=p<.5?2*p*p:-1+(4-2*p)*p;angle=m360(st+df*p);upd(angle);
...
function loop(){if(!spinning)return;angle=m360(angle+1.3);upd(angle);requestAnimationFrame(loop);}
var sv=document.getElementById('av');
if(sv){
  sv.addEventListener('mousedown',function(e){spinning=false;var b=document.getElementById('sb');if(b)b.textContent='▶ Spin';dragX=e.clientX;dragA=angle;e.preventDefault();});
  document.addEventListener('mousemove',function(e){if(dragX===null)return;angle=m360(dragA+(e.clientX-dragX)*0.55);upd(angle);});
  ...
  document.addEventListener('touchmove',function(e){if(dragX===null)return;angle=m360(dragA+(e.touches[0].clientX-dragX)*0.55);upd(angle);e.preventDefault();},{passive:false});
