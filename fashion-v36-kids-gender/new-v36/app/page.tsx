...
export default function FashionStylist() {
  ...
  const [subCategory, setSubCategory] = useState<'Boy'|'Girl'|null>(null)
  ...
  const [tryOnMethod, setTryOnMethod]        = useState<string>('IDM-VTON')
  ...
  const analyze = async (file: File) => {
    ...
    try {
      ...
      if (category === 'Kids' && subCategory) form.append('sub_category', subCategory)
  ...
  // ── IDM-VTON via yisol/IDM-VTON on HuggingFace (FREE, photorealistic) ──
  const doTryOn = async () => {
    if (!photo || !dressFile) return
    setTryOnLoading(true); setTryOnImage(null); setError('')

    try {
      const fileToDataUri = (f: File | Blob): Promise<string> =>
        new Promise((res, rej) => {
      ...
      setTryOnLoadingMsg('Preparing images...')
      const personDataUri = await fileToDataUri(photo)

      // Use recolored dress data URI if a color was selected, else original file
      let garmentDataUri: string
      if (selectedColor && coloredDataUri) {
        garmentDataUri = coloredDataUri   // already a data URI from canvas recolor
      } else {
        garmentDataUri = await fileToDataUri(dressFile)
      }

      // ── Call /api/tryon — server route calls yisol/IDM-VTON for free ──
      setTryOnLoadingMsg('Joining IDM-VTON queue on HuggingFace...')
      ...
      setTryOnLoadingMsg('IDM-VTON generating photorealistic result...')
      const data = await resp.json()

      if (data.error) {
        // Canvas fallback on error
        setTryOnLoadingMsg('Falling back to canvas overlay...')
        const garmentSrc = (selectedColor && coloredDataUri) ? coloredDataUri : URL.createObjectURL(dressFile)
        const fallback   = await canvasFallback(photo, garmentSrc)
        setTryOnImage(fallback)
        setError(`⚠️ IDM-VTON: ${data.error}`)
      } else {
        setTryOnImage(data.output_url)
        setTryOnMethod(data.method ?? 'IDM-VTON')
        if (selectedColor) {
            ...
            <div style={{ background: '#12103a', border: '1px solid #2a2060', borderRadius: 18, padding: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                {['Women','Men','Kids'].map(c => (
                  <button key={c} onClick={() => { setCategory(c); if (c !== 'Kids') setSubCategory(null) }} style={{ flex: 1, padding: '9px 0', border: `1.5px solid ${category===c?'#8060e0':'#1e1848'}`, borderRadius: 9, cursor: 'pointer', fontWeight: 700, fontSize: 12, background: category===c?'#2a1f60':'#0d0d2a', color: category===c?'#e8c99a':'#5040a0' }}>{c}</button>
                ))}
              </div>

              {category === 'Kids' && (
                <div style={{ marginBottom: 14 }}>
                  <div style={{ color: '#7060a0', fontSize: 11, marginBottom: 8, fontWeight: 600 }}>Select Gender</div>
                  <div style={{ display: 'flex', gap: 10 }}>
                    {(['Girl', 'Boy'] as const).map(g => (
                      <button key={g} onClick={() => setSubCategory(g)} style={{
                        flex: 1, padding: '10px 0', border: `1.5px solid ${subCategory===g?'#8060e0':'#1e1848'}`,
                        borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: 13,
                        background: subCategory===g?'#2a1f60':'#0d0d2a',
                        color: subCategory===g?'#e8c99a':'#5040a0'
                      }}>
                        {g === 'Girl' ? '👧 Girl' : '👦 Boy'}
                      </button>
                    ))}
              ...
              <div onClick={category === 'Kids' && !subCategory ? undefined : openPhotoPicker}
                ...
                {category === 'Kids' && !subCategory
                  ? <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, opacity: 0.5 }}>
                      ...
                      <div style={{ color: '#5050a0', fontSize: 13, textAlign: 'center' }}>Select <b style={{color:'#c0a0ff'}}>Girl</b> or <b style={{color:'#c0a0ff'}}>Boy</b> above before uploading</div>
                    </div>
                  : photoPreview
                  ? <>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(310px,1fr))', gap: 20 }}>
              ...
              <div>
                ...
                <div style={{ marginTop: 10, padding: '10px 14px', background: '#0d0d22', border: '1px solid #2a2060', borderRadius: 10, fontSize: 11, lineHeight: 1.8 }}>
                  <div style={{ marginBottom: 6, color: '#c0a0ff', fontWeight: 700, fontSize: 12 }}>⚡ 4-Tier Smart Fallback — Always Gets a Result</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ background: '#1e1048', color: '#8060c0', fontSize: 10, padding: '1px 7px', borderRadius: 4, fontWeight: 700, minWidth: 145, textAlign: 'center' }}>① IDM-VTON · Free</span>
                      <span style={{ color: '#5a6898' }}>Photorealistic · free · ~5-10/day</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ background: '#0a1a0a', color: '#4ade80', fontSize: 10, padding: '1px 7px', borderRadius: 4, fontWeight: 700, minWidth: 145, textAlign: 'center' }}>② SegFit · 100 Free/day</span>
                      <span style={{ color: '#5a6898' }}>Photorealistic · <b style={{color:'#4ade80'}}>100 free calls/day</b> · ~₹1 after</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ background: '#1a0d00', color: '#f97316', fontSize: 10, padding: '1px 7px', borderRadius: 4, fontWeight: 700, minWidth: 145, textAlign: 'center' }}>③ IDM-VTON · Replicate</span>
                      <span style={{ color: '#5a6898' }}>Photorealistic · ~₹4/try-on · unlimited</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ background: '#0d2010', color: '#22c55e', fontSize: 10, padding: '1px 7px', borderRadius: 4, fontWeight: 700, minWidth: 145, textAlign: 'center' }}>④ HF Space · YOLOv8</span>
                      <span style={{ color: '#5a6898' }}>Free · unlimited · basic quality</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                {tryOnLoading && (
                  <div style={{ background: '#12103a', border: '1px solid #2a2060', borderRadius: 16, padding: 32, textAlign: 'center', marginBottom: 14 }}>
                    ...
                    {['Recoloring dress to ' + (selectedColor||'selected color'), 'Joining IDM-VTON queue on HuggingFace...', 'Diffusion model generating photorealistic result'].map((s, i) => (
                ...
                {tryOnImage && !tryOnLoading && (
                  <div style={{ background: '#12103a', border: '1px solid #2a2060', borderRadius: 16, overflow: 'hidden', marginBottom: 14 }}>
                    <div style={{ padding: '12px 16px', borderBottom: '1px solid #1e1848', display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{
                        marginLeft: 'auto',
                        background: tryOnMethod.includes('YOLOv8') ? '#0d2010' : tryOnMethod.includes('Replicate') ? '#1a0d00' : '#1e1048',
                        color: tryOnMethod.includes('YOLOv8') ? '#22c55e' : tryOnMethod.includes('Replicate') ? '#f97316' : '#8060c0',
...
function SizeCard({ result, category }: { result: any; category: string }) {
  const sizeMap: Record<string, Record<string, string>> = {
    ...
    'Kids-Girl': { '2Y':'H≤92','3Y':'H≤100','4Y':'H≤108','5Y':'H≤115','6Y':'H≤122','7Y':'H≤128','8Y':'H≤134','10Y':'H≤140','12Y+':'H>140' },
    'Kids-Boy':  { '2Y':'H≤94','3Y':'H≤102','4Y':'H≤110','5Y':'H≤117','6Y':'H≤124','7Y':'H≤130','8Y':'H≤136','10Y':'H≤143','12Y+':'H>143' },
  }
  const kidsKey = result.sub_category === 'Boy' ? 'Kids-Boy' : 'Kids-Girl'
  const chart = category === 'Kids' ? (sizeMap[kidsKey] || sizeMap['Kids-Girl']) : (sizeMap[category] || sizeMap.Women)
  return (
    <div style={{ background: '#12103a', border: '1px solid #2a2060', borderRadius: 14, overflow: 'hidden' }}>
      <div style={{ background: 'linear-gradient(135deg,#2a1060,#1a0848)', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          ...
          <div style={{ color: '#6050a0', fontSize: 11 }}>Bust {result.bust_cm}cm · {category}{result.sub_category ? ` · ${result.sub_category}` : ''}</div>
        </div>
      </div>
