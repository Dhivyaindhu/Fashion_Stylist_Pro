...
const API = process.env.NEXT_PUBLIC_API_URL || 'https://indhu321-fashion-stylist-app.hf.space'
...
// ═══════════════════════════════════════════════════════════════
// INLINE SVG FACE — always friendly, skin-adaptive, no external API
// Women: long hair + lashes + blush  |  Men: short hair + thick brows
// ═══════════════════════════════════════════════════════════════
function drawFaceSVG(cx:number, cy:number, r:number, xsh:number, skinHex:string, skinTone:string, isMen:boolean): string {
  ...
  // face oval
  const oval=`<ellipse cx="${x}" cy="${cy}" rx="${s}" ry="${s*1.09}" fill="${skinHex}"/>`
  // eyes
  ...
  // nose
  const ny=cy+s*.27
  const ns=`<path d="M ${x-s*.065},${ny-s*.04} Q ${x},${ny+s*.05} ${x+s*.065},${ny-s*.04}" fill="none" stroke="rgba(0,0,0,0.18)" stroke-width="${s*.042}" stroke-linecap="round"/>`
  // mouth — always upward smile
  const my=cy+s*.46
  const mt=`<path d="M ${x-s*.195},${my} Q ${x},${my+s*.14} ${x+s*.195},${my}" fill="${lp}" stroke="${lp}" stroke-width="${s*.036}" stroke-linecap="round"/><path d="M ${x-s*.195},${my} Q ${x},${my-s*.03} ${x+s*.195},${my}" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="${s*.02}"/>`
  // blush (women only)
...
// ═══════════════════════════════════════════════════════════════
// BUILD AVATAR  — uses real face photo if available, else SVG face
// Better virtual try-on: dress image warped to body silhouette
// ═══════════════════════════════════════════════════════════════
function buildAvatar(result: any, dressB64: string|null, faceB64: string|null): string {
  ...
  const CX=210, W=420, H=720, SC=4.8
  ...
  const y_sh=220, y_bu=y_sh+70, y_wa=y_bu+60, y_hi=y_wa+42
  const y_th=y_hi+80, y_kn=y_th+60, y_ca=y_kn+54, y_ft=y_ca+48
  const y_nek=y_sh-24, y_hcy=y_nek-80

  const skin_sh=lighten(skin,0.68), skin_hi=lighten(skin,1.32), skin_mid=lighten(skin,0.84)

  // ── FACE: real photo texture OR inline SVG ─────────────────────────
  const headR = 70
  let faceLayer = ''
  if (faceB64) {
    // Real face photo — clipped to circle
    faceLayer = `<clipPath id="fClip"><circle cx="${CX}" cy="${y_hcy}" r="${headR}"/></clipPath>
<image id="faceImg" href="data:image/jpeg;base64,${faceB64}" x="${CX-headR}" y="${y_hcy-headR}" width="${headR*2}" height="${headR*2}" clip-path="url(#fClip)" preserveAspectRatio="xMidYMid slice"/>`
  } else {
    // Inline SVG face — always friendly
    faceLayer = `<g id="faceG">${drawFaceSVG(CX, y_hcy, headR, 0, skin, skinTone, isMen)}</g>`
  }
  ...
  function dressP(sw:number,bw:number,ww:number,hw_:number,sh:number) {
    const sw2=sw+20, bw2=bw+14, ww2=ww+6, hw2=hw_+8
    const L=(v:number)=>CX-v+sh, R=(v:number)=>CX+v+sh
    return `M ${L(sw2)},${y_sh-4} C ${L(sw2+10)},${y_sh+22} ${L(bw2+6)},${y_bu-16} ${L(bw2)},${y_bu} C ${L(bw2-6)},${y_bu+26} ${L(ww2+5)},${y_wa-14} ${L(ww2)},${y_wa} C ${L(ww2+6)},${y_wa+20} ${L(hw2-4)},${y_hi-12} ${L(hw2)},${y_hi} C ${L(hw2+2)},${y_hi+32} ${L(hw2+6)},${y_ft-12} ${L(hw2)},${y_ft} L ${R(hw2)},${y_ft} C ${R(hw2+6)},${y_ft-12} ${R(hw2+2)},${y_hi+32} ${R(hw2)},${y_hi} C ${R(hw2-4)},${y_hi-12} ${R(ww2+6)},${y_wa+20} ${R(ww2)},${y_wa} C ${R(ww2+5)},${y_wa-14} ${R(bw2-6)},${y_bu+26} ${R(bw2)},${y_bu} C ${R(bw2+6)},${y_bu-16} ${R(sw2+10)},${y_sh+22} ${R(sw2)},${y_sh-4} Z`
  }
  function armP(s:number,sw:number,sh:number) {
    const ax=CX+s*sw+sh, ay=y_sh+10, ex=CX+s*(sw+30)+sh, ey=y_sh+108, hx=CX+s*(sw+12)+sh, hy=y_sh+210
    return `M ${ax},${ay} C ${ax+s*18},${ay+28} ${ex-s*5},${ey-24} ${ex},${ey} C ${ex+s*4},${ey+34} ${hx+s*10},${hy-34} ${hx},${hy}`
  }
  function neckP(nn:number,sh:number) {
    return `M ${CX-nn+sh},${y_nek+4} C ${CX-nn+2+sh},${y_nek+16} ${CX-nn+3+sh},${y_sh-6} ${CX-nn+4+sh},${y_sh} L ${CX+nn-4+sh},${y_sh} C ${CX+nn-3+sh},${y_sh-6} ${CX+nn-2+sh},${y_nek+16} ${CX+nn+sh},${y_nek+4} Z`
  }
  ...
  const dressImgW=(hi_w+36)*2, dressImgH=y_ft-y_sh+28

  const dressDefs = dressB64
    ? `<clipPath id="dCl"><path id="dClP" d="${initDr}"/></clipPath>` : ''

  // ── DRESS LAYER: better fit using body-matched clip path ───────────
  const dressLayer = dressB64
    ? `<image id="dImg" href="data:image/png;base64,${dressB64}" x="${CX-dressImgW/2}" y="${y_sh-4}" width="${dressImgW}" height="${dressImgH}" clip-path="url(#dCl)" preserveAspectRatio="xMidYMid slice" opacity="0.97"/>
       <path d="${initDr}" fill="none" stroke="rgba(0,0,0,0.06)" stroke-width="1"/>`
    : ''
...
<style>*{margin:0;padding:0;box-sizing:border-box}body{background:#07071a;display:flex;justify-content:center;padding:8px;font-family:system-ui}svg{cursor:grab;touch-action:none;user-select:none}</style>
</head><body>
...
<filter id="ds"><feDropShadow dx="2" dy="5" stdDeviation="6" flood-opacity="0.28"/></filter>
${dressDefs}
</defs>
<rect width="${W}" height="${H}" fill="url(#bgG)"/>
<ellipse cx="${CX}" cy="${y_ft+18}" rx="${hi_w+16}" ry="13" fill="rgba(0,0,0,0.2)" filter="url(#bl)"/>
...
<path d="M ${CX},${y_sh+6} C ${CX},${y_bu-6} ${CX},${y_bu+18} ${CX},${y_wa}" fill="none" stroke="rgba(255,255,255,0.10)" stroke-width="10" stroke-linecap="round"/>
${dressLayer}
${sleeveLayer}
<path id="neck" d="${initNk}" fill="${skin_mid}" filter="url(#ds)"/>
<circle id="headCirc" cx="${CX}" cy="${y_hcy}" r="${headR}" fill="${skin}" filter="url(#ds)"/>
${faceLayer}
...
var NW=${nw},ARW=${arm_w},AH=${ah},HR=${headR};
var YSH=${y_sh},YBU=${y_bu},YWA=${y_wa},YHI=${y_hi},YTH=${y_th},YKN=${y_kn},YCA=${y_ca},YFT=${y_ft};
var YNER=${y_nek},YHCY=${y_hcy};
var BT="${bodyType}",hasDress=${dressB64?'true':'false'},hasFace=${faceB64?'true':'false'};
var DW=${dressImgW};
...
function dressPath(sw,bw,ww,hw,sh){
  var sw2=sw+20,bw2=bw+14,ww2=ww+6,hw2=hw+8;
  var L=function(v){return CX-v+sh;},R=function(v){return CX+v+sh;};
  return 'M '+L(sw2)+','+(YSH-4)+' C '+L(sw2+10)+','+(YSH+22)+' '+L(bw2+6)+','+(YBU-16)+' '+L(bw2)+','+YBU+' C '+L(bw2-6)+','+(YBU+26)+' '+L(ww2+5)+','+(YWA-14)+' '+L(ww2)+','+YWA+' C '+L(ww2+6)+','+(YWA+20)+' '+L(hw2-4)+','+(YHI-12)+' '+L(hw2)+','+YHI+' C '+L(hw2+2)+','+(YHI+32)+' '+L(hw2+6)+','+(YFT-12)+' '+L(hw2)+','+YFT+' L '+R(hw2)+','+YFT+' C '+R(hw2+6)+','+(YFT-12)+' '+R(hw2+2)+','+(YHI+32)+' '+R(hw2)+','+YHI+' C '+R(hw2-4)+','+(YHI-12)+' '+R(ww2+6)+','+(YWA+20)+' '+R(ww2)+','+YWA+' C '+R(ww2+5)+','+(YWA-14)+' '+R(bw2-6)+','+(YBU+26)+' '+R(bw2)+','+YBU+' C '+R(bw2+6)+','+(YBU-16)+' '+R(sw2+10)+','+(YSH+22)+' '+R(sw2)+','+(YSH-4)+' Z';
}
function armPath(s,sw,sh){
  var ax=CX+s*sw+sh,ay=YSH+10,ex=CX+s*(sw+30)+sh,ey=YSH+108,hx=CX+s*(sw+12)+sh,hy=YSH+210;
  return 'M '+ax+','+ay+' C '+(ax+s*18)+','+(ay+28)+' '+(ex-s*5)+','+(ey-24)+' '+ex+','+ey+' C '+(ex+s*4)+','+(ey+34)+' '+(hx+s*10)+','+(hy-34)+' '+hx+','+hy;
}
function neckPath(nn,sh){
  return 'M '+(CX-nn+sh)+','+(YNER+4)+' C '+(CX-nn+2+sh)+','+(YNER+16)+' '+(CX-nn+3+sh)+','+(YSH-6)+' '+(CX-nn+4+sh)+','+YSH+' L '+(CX+nn-4+sh)+','+YSH+' C '+(CX+nn-3+sh)+','+(YSH-6)+' '+(CX+nn-2+sh)+','+(YNER+16)+' '+(CX+nn+sh)+','+(YNER+4)+' Z';
}
function upd(a){
  a=m360(a);var r=a*Math.PI/180,cosA=Math.cos(r),sinA=Math.sin(r);
  var wS=Math.abs(cosA)*0.84+0.16,sh=Math.round(sinA*24);
  ...
  S('headCirc','cx',CX+sh);
  // Face rotation — handles both real photo and SVG face
  if(hasFace){
    var fi=document.getElementById('faceImg');
    if(fi){fi.setAttribute('x',CX-HR+sh);fi.style.opacity=Math.max(0,cosA).toFixed(2);}
    var fc=document.getElementById('fClip');
    if(fc){var cc=fc.querySelector('circle');if(cc)cc.setAttribute('cx',CX+sh);}
  } else {
  ...
  if(hasDress){
    ...
    if(di){
      ...
      di.style.opacity=(0.52+Math.max(0,cosA)*0.46).toFixed(2);
    }
...
export default function Home() {
  ...
  // ── Extract real face from uploaded photo ─────────────────────────
  const extractFace = async (file: File) => {
    try {
      const form = new FormData()
      form.append('file', file)
      const res = await fetch(`${API}/extract-face`, { method:'POST', body:form })
      const data = await res.json()
      if (data.success && data.face_b64) {
        setFaceB64(data.face_b64)
      }
    } catch { /* face extraction is optional — SVG fallback used */ }
  }

  const analyze = async (file: File) => {
    setLoading(true); setError(''); setFaceB64(null)
    ...
    try {
      ...
      setStep('result')
      // Try to extract real face after analysis completes
      extractFace(file)
  ...
  // ── Fit badges using standard size charts ────────────────────────
  const WOMEN_BUST: Record<string,number> = {XS:80,S:84,M:88,L:94,XL:100,XXL:106,XXXL:114,'4XL':122}
  const MEN_BUST: Record<string,number>   = {S:92,M:97,L:102,XL:108,XXL:114,XXXL:122}
  const fitBadges = () => {
    if (!result) return null
    const bustMap = result.category==='Men' ? MEN_BUST : WOMEN_BUST
    const std = bustMap[result.size] ?? result.bust_cm
    return [
      ...
      return (
        <div key={zone as string} style={{display:'flex',alignItems:'center',gap:8,padding:'7px 10px',background:'#07071a',borderLeft:`4px solid ${col}`,borderRadius:6,marginBottom:5,fontSize:12}}>
          <span>{icon}</span>
          <div><div style={{color:col,fontWeight:700}}>{zone as string}</div>
          <div style={{color:'#555',fontSize:11}}>{lbl} ({d>=0?'+':''}{d.toFixed(1)} cm ease)</div></div>
        </div>
      )
    })
  }
  ...
  const avatarFrame = (id: string) => {
    const frameKey = `${id}-${dressB64?dressB64.slice(-12):'bare'}-${faceB64?faceB64.slice(-8):'noface'}`
    return (
      <div style={{background:'#08081a',borderRadius:16,overflow:'hidden',minHeight:480}}>
        <iframe key={frameKey} srcDoc={result ? buildAvatar(result, dressB64, faceB64) : ''}
          style={{width:'100%',height:600,border:'none',display:'block'}} title="avatar"/>
      </div>
    )
  }
    ...
    <main style={{minHeight:'100vh',background:'#06061a',color:'#e8e0ff',fontFamily:'system-ui,sans-serif'}}>
      ...
      <div style={{background:'linear-gradient(135deg,#1a0938,#0d0628)',padding:'18px 24px',borderBottom:'1px solid #1e1848',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
        <div>
          ...
          <p style={{margin:'2px 0 0',color:'#7060a0',fontSize:'0.76rem'}}>AI body analysis · 3D avatar · Virtual try-on · Smart recommendations</p>
        </div>
        {result && (
          <div style={{display:'flex',alignItems:'center',gap:10,background:'#1a1848',border:'1px solid #2e2868',borderRadius:12,padding:'7px 14px'}}>
            ...
            {result.clothing_type && result.clothing_type!=='Unknown' && result.clothing_type!=='Fitted' && (
              <span style={{fontSize:10,color:'#ffcc00',background:'rgba(255,200,0,.12)',padding:'2px 6px',borderRadius:4}}>🥻 {result.clothing_type}</span>
            )}
          </div>
        )}
      </div>
        ...
        {step==='upload' && (
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(290px,1fr))',gap:18}}>
            <div style={{background:'#10103a',border:'1px solid #2a2860',borderRadius:16,padding:22}}>
              ...
              <div style={{color:'#5050a0',fontSize:12,marginBottom:14}}>Stand straight, facing camera, head to toe visible</div>
              ...
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14,background:'#0d0d2a',borderRadius:10,padding:'8px 12px',border:'1px solid #1e1848'}}>
                ...
                <span style={{color:'#7060a0',fontSize:12,whiteSpace:'nowrap'}}>Your height</span>
              ...
              {loading && <div style={{marginTop:12,color:'#8060e0',fontSize:13,textAlign:'center',padding:10,background:'#1a1848',borderRadius:8}}>⏳ Analysing with YOLO pose detection...</div>}
            ...
            <div style={{background:'#10103a',border:'1px solid #1e1848',borderRadius:16,padding:22}}>
              ...
              {[
                ['📏','Accurate body measurements (shoulder, bust, waist, hip)'],
                ['👗','Size — Indian standard chart (XS to XXXL)'],
                ['🎨','Personalised colour palette for your skin tone'],
                ['🤖','3D avatar with YOUR real face texture (auto-extracted!)'],
                ['🪄','Virtual try-on — see clothes fitted on your exact body shape'],
                ['🛍','Shopping links with your size on Amazon & Flipkart'],
                ['🥻','Saree & ethnic wear aware — no more wrong sizing!'],
              ].map(([e,t])=>(
              ...
              <div style={{marginTop:14,padding:'12px 14px',background:'#0d0d22',border:'1px solid #1a1848',borderRadius:10,fontSize:12,color:'#4a4880'}}>
                💡 <b style={{color:'#6a6898'}}>Best results:</b> Full body in frame, arms slightly away from body, plain background, good lighting
              </div>
            </div>
          </div>
        )}
        ...
        {step==='result' && result && (
          <div>
            <div style={{display:'flex',borderBottom:'1px solid #1e1848',marginBottom:18,overflowX:'auto'}}>
              ...
              {tabBtn('tryon','👗 Try-On')}
              {tabBtn('shop','🛍 Shop')}
              <button onClick={()=>{setStep('upload');setResult(null);setPreview(null);setFaceB64(null);clearDress()}}
                style={{marginLeft:'auto',padding:'8px 14px',background:'#1a1848',color:'#6050a0',border:'1px solid #2a2860',borderRadius:8,cursor:'pointer',fontSize:12}}>📸 New Photo</button>
            </div>
            ...
            {activeTab==='avatar' && (
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:18}}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:14}}>
                  {visImg && (
                    <img src={visImg} alt="detection" style={{width:'100%',borderRadius:12,border:'1px solid #2a2860',maxHeight:280,objectFit:'contain',background:'#000'}}/>
                  )}
                  <div style={{background:'#10103a',border:'1px solid #2a2860',borderRadius:14,padding:16}}>
                    <div style={{color:'#e8c99a',fontWeight:800,marginBottom:6}}>
                      {result.body_icon} {result.body_type}
                      <span style={{marginLeft:8,background:'#2a1f60',padding:'2px 10px',borderRadius:6,fontSize:14}}>{result.size}</span>
                    </div>
                    {result.clothing_type && result.clothing_type!=='Unknown' && result.clothing_type!=='Fitted' && (
                      <div style={{background:'rgba(255,200,0,.10)',border:'1px solid #ffcc0040',borderRadius:6,padding:'4px 10px',display:'inline-block',fontSize:11,color:'#ffcc00',marginBottom:8}}>
                        🥻 {result.clothing_type} — measurements adjusted for drape
                      </div>
                    )}
                    {faceB64 && <div style={{color:'#22c55e',fontSize:10,marginBottom:4}}>✅ Real face extracted from your photo</div>}
                    <div style={{color:'#4040a0',fontSize:10,marginBottom:4}}>{result.method} · {result.confidence}% confidence</div>
                    ...
                    <div style={{marginTop:10,color:'#3a3060',fontSize:10,textAlign:'center'}}>±3cm estimated · 2D photo, depth not available</div>
                  </div>
                  <div style={{background:'#10103a',border:'1px solid #2a2860',borderRadius:14,padding:16}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:18}}>
                <div style={{display:'flex',flexDirection:'column',gap:14}}>
                  <div style={{background:'#10103a',border:'1px solid #2a2860',borderRadius:16,padding:18}}>
                    ...
                    <div style={{color:'#5050a0',fontSize:12,marginBottom:12}}>White-background product photos give the best fit overlay</div>
                    <div onClick={()=>dressRef.current?.click()} style={{border:'2px dashed #2a2860',borderRadius:12,padding:18,cursor:'pointer',background:'#0d0d2a',textAlign:'center',minHeight:160,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:8}}>
                  ...
                  <div style={{background:'#0d0d22',border:'1px solid #1e1848',borderRadius:14,padding:16}}>
                    ...
                    {fitBadges()}
                    <div style={{marginTop:8,padding:'8px 10px',background:'#07071a',borderRadius:8,fontSize:11,color:'#5050a0',textAlign:'center'}}>0–6cm ease = perfect fit for most styles</div>
                  </div>
                  <div style={{background:'#10103a',border:'1px solid #2a2860',borderRadius:12,padding:14,fontSize:12,color:'#5050a0'}}>
                    ...
                    <p style={{margin:'3px 0'}}>• Drag avatar left/right to see from all angles</p>
                    <p style={{margin:'3px 0'}}>• Avatar face uses YOUR photo when detected</p>
                  </div>
                </div>
                {avatarFrame('av2')}
              </div>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
          {bestColors.slice(0,8).map((c:string)=>(
            <span key={c} style={{display:'inline-flex',alignItems:'center',gap:3,background:'#1e1848',color:'#a090d0',border:'1px solid #2e2868',borderRadius:8,padding:'2px 8px',fontSize:11}}>
              <span style={{width:8,height:8,borderRadius:'50%',background:COLOR_HEX[c]||'#888',display:'inline-block'}}/>{c}
            </span>
          ))}
        </div>
      </div>
