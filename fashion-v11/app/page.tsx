...
function upd(a){
  ...
  if(hasDress){
    ...
    if(di){var sW=DW*wS;di.setAttribute('width',sW.toFixed(1));di.setAttribute('x',(CX-sW/2+sh).toFixed(1));}
...
export default function Home() {
  ...
  const avatarFrame = (key: string) => (
    <div style={{background:'#08081a',borderRadius:16,overflow:'hidden',minHeight:480}}>
      <iframe key={key} srcDoc={result?buildAvatar(result,dressB64):''} style={{width:'100%',height:560,border:'none',display:'block'}} title="avatar"/>
    </div>
  )
